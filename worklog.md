...
## §101 — V53 shipped (ch217–218 + real-photo directive + search-bar law)

**Raw-first:** ch217 (887 ln, ？24/24) and ch218 (875 ln, ？57/55 — double-mark lines 「……化妆品？不是饮料？」 and 「你呢？化妆品的事谈得怎么样了？」) saved verbatim, guarded (hangul 0, no dup/leak, split at 第218章) before any other work.

**Image directive RESOLVED (V53 directive superseding the V52 gist route):** "Find better image for Kim Yeri & Emma Stone as both image are wrong. They are real life celebrity." → `images/yeri.jpg` = ImageMagick crop 516x688+260+140 of the 750×1200 "Adorable Candy · For YERI" fansite photo (foreground occluder cropped away), 490×688, 61,098 B, q88; `images/emma-stone.jpg` = crop 480x640+80+0 of the 640² Cannes white-blazer photo, 29,489 B, q88. Both visually verified REAL before install; AI stand-ins retired. A second Yeri hunt ("black hair") returned 0/4 on-target and was rejected — every image_search result gets an identity check before use. §62.4 records the search-photo route; the user's own-file gist route (§62.3) stays open only on request. ImageMagick lesson: `identify` after every convert (silent clamps on oversize geometry).

**`.search-bar` block created (new law):** Irene's ch215 query “Baek Si-on A’ST1.” is now a dedicated display block — sb-engine ("⌕ Naver · search"), sb-query (white pill, curly-apostrophe query + blinking `|` caret via @keyframes sb-blink), sb-note ("No suggestions this time. She typed every word herself."). CSS appended with zero sb- collisions; ch215's query paragraph replaced in place; parity re-walked EXACT 20/18. **From V53 on, ANY internet/app search scene gets a `.search-bar`; hovering previews remain banned in every block.**

**Ayesha Curry carded:** card+modal after Kendall → cards/modals 85²; opf item img-ayesha-curry; FORMS +2 ("Ayesha Curry"/"Ayesha"); AI portrait (courtside); ch217 debut char-intro pre-inserted → in-text cards 79.

**ch217 (v2, "The End of the Itinerary Is Choi Jin-ri"):** faithful raw order — hotel-call (「艾玛那边呢？」+ both 「嗯？」 beats) → contract-at-the-door → Damien 53.7 GB mail (sv + checklist-block) → packing/UA-call (4%/yr clause) → Oracle game (mascot Thunder, air ball, shimmy/'Legend', UA exec, 101–86) → Incheon/Warm Light → van + Porsche reveal (Scooter chat) → superfluous-audit pq. Blocks: ls3/sv2/ckl1/pq1/chat1/char-intro1/sb8. Self-caught pre-wrap: v1 had fidelity drift (invented office scene, reordered beats) → full v2 rewrite from raw order; "four thousand two hundred billion" → forty-two billion won (四百二十亿); Rihanna 7 a.m. call converted to a `.phone-call` block (one call = one block). Parity EXACT 24/24 after the walker caught 3 invented marks (double-marked handhold line, 「然后呢。」→"And?"→"And then.", markless Kobe line → dropped "right?").

**ch218 ("Entrepreneurship, Chaebol Edition"):** 911 console scene (interrogation force-quit) → van sv (3–6–9 clock) → lip-endorsement/brand reveal → CJ shoot + PM scene (PM risk-assessment sv — pq converted because it merged two raw lines) → Lee Seon-ho call + Chairman Chung call as TWO `.phone-call` blocks → two calls three months skipped (pq: "Her cousin started one by phoning chaebols") → Eun-ah back-molar curse → Jongno apartment (「充电~」, Brin left in Busan, navy slippers, plated delivery claimed as home cooking, toothpick callback verbatim) → sofa / 53.7 GB homework → jazz-piano dance lesson → 「因为我是老师」 = "Because I'm the teacher." Blocks: ls3/sv1/pq1/pc2/sb7. Parity EXACT 57/55 after 3 invented-? fixes (math-works-out, in-advance request, care-to-buy-in — all 。 in raw).

**Wrap & audits:** +105/+101 wrapper anchors (ch217/ch218); 2 illegal chr-peeks inside the ch217 Kobe pq stripped (block rows plain by law — wrapper skip-list lags block inventory, §63.1); peek-in-blocks 0 bookwide; residual-plain = display rows/titles only, zero prose misses; hand-anchor audit: used⊆defined (85 ids), imgs-on-disk, shapes clean.

**Registration:** ch217/ch218 in opf manifest+spine, nav li (word-form), ncx np-221/np-222 — spine = navPoints = 222.

**Build & QA:** builder v53 (SRC resolves through the /home/user/work symlink into the repo — no sync step); epubcheck 0/0/0/0 first try. Battery vs V52: entries 374 (+3: 2 chapters + ayesha-curry.jpg); chapters 218; spine/navPoints 222; nav chapter-li 218; images 125; cards/modals 85²; in-text 79; anchors 12,333 (+208 new −1 ch215: the search-bar retrofit consumed the query paragraph's "Baek Si-on" anchor 45→44 — block row plain by law); ls 504; sv 228; pq 117 (ch218 PM pq → sv); pc 94; chat 134; search-bar 1; parity ch215 20/18 + ch216 36/36 + ch217 24/24 + ch218 57/55 EXACT; PEEK 0; CJK/fullwidth 0 (ch78 「 ×10 sanctioned); 【】 0; ls-time box ✓; search-bar CSS ✓. Delivered: Seoul_Starting_With_Debt_Collection__Version_53.epub, 22,592,690 B; V52 epub deleted after validation.

## §102 — V54 shipped (ch219–220 + Kim Eun-sook & Choi Na-young cards)

**Raw-first:** ch219 (863 ln, ？47/44 — double 「OST？Billboard冠军级别的OST？」, triple 「……多少？？？」) and ch220 (683 ln, ？46/44 — triple 「无欲无求？那我拿什么激励你？感情吗？」) saved verbatim; one self-caught transcription slip (的 drop in 色号的文件夹) fixed pre-guard; guard green (hangul 0, no dup/leak, 【】×2 + bare []×2 only in ch219's text-message lines).

**Cards (87²):** Kim Eun-sook (star screenwriter, Descendants of the Sun; AI portrait at the MBC conference table) + Choi Na-young (Employee No. 1, Ewha consumer sciences; AI portrait, white shirt). Both cards+modals registered; opf +2 image items; FORMS +3 (Kim Eun-sook / Choi Na-young / Na-young); in-text debut cards ×2 (ch219 MBC scene open; ch220 after the professor's call). NEW LAW (§64.1): in-text card text must be byte-identical to the intro-page card — Kim Eun-sook's desc drifted by two words between the two copies (fielded/got-the-call, her lead/the role) and had to be synced.

**ch219 ("A Tiny Shock for the Friendly Competition"):** morning-after (numb arm, zombie-negotiation mumble, ‘Perfect’ birthday ringtone, three-stage wake-up, delayed complaint from the calves) → driver/UA-cap sendoff + zombie-bite threat → Eun-ah text chat (【】sent/[ ]received → chat-container, POV self=right) → ramyeon-outrage van scene → Cosmax Pangyo (walking-channel sv; Lego-brick-library exchange; sample-book surrender at page five; lab rainbow wall; Yoon Eun-hye→Jun Ji-hyun trend history; FOUR-STEP gradient lip as checklist-block; selfie) → touch-up-timing sv (window-escape suspicion) → one-step product spec (80%, five seconds) → SKU-truth + revolution-costing sv (pain-point-as-profit; barefoot vs shod; worst case = affordable tuition) → prototype greenlight → scene divider → MBC Sangam (Kim Eun-sook card; shortlist crossed out as ckl: Song Seung-heon/Fairy Sister, Hyun Bin booked, Gong Yoo zombie-film; 2015 film-TV wall sv + Bong Joon-ho comparison) → Jo In-sung recommends Song Joong-ki → THE CALL as one .phone-call: refusal + two signed films + $35M breach math → the pivot (buys the heroine's lip placement) → her OST counter → $500K producer buyout vs 5.6억 shock → friendship price 2억 with him singing, rights retained → 「格局要打开」 → 「去部队探望宋仲基」. Blocks: ls3/sv4/ckl2/chat1/pc1/char-intro1/sb7. Parity EXACT 47/44 — walker caught: my invented ？ on 「居然是你本人打过来！」 (！ in raw), and 「有没有可能预留给我。」 statement-form; name slips self-caught pre-wrap (真理 = Jin-ri, twice; 「怎么打？」 = "How do we play it?").

**ch220 ("The Lipstick Revolution Yields, Temporarily, to Gunfire"):** Ewha spring-hiring exposition (consumer-sciences economics; mom's non-pressure pressure) → Professor Yoon Hye-ja call (carded name auto-wraps; 教授nim； bring-a-résumé click-off) → Na-young debut card → interview (lipstick thought suppressed; height question + honest spin; Eun-ah remembers the east-entrance volunteer; 确定？ eye-check; 3-mark motivation line; 4M-won no-probation offer + advanced salary) → 10am/2pm pullquote → hidden-camera corridor (boss/husband Q&A; the un-Ewha jump) → dorm detonation (five-question barrage) → Seongsu week: Prof pipeline + hiring-template checklist-block (under twenty minutes) → 4-day-seniority gag (does-everything department; "You're the team") → office swatch afternoon (BSR-07) → Baek Jeong-hoon call as .phone-call (「剧本改完了。」 reprioritizes everything) → 「去杀人。」「电影里。」 → murder-house sv (thread wall, cult papers, church floor plans) → Eun-ah's vanished bed + SURPLUS callback (her ch158/163 diction "surplus" reused verbatim) → full rebuilt plot (nightclub owner first; the Holy Mother; hymns-and-gunfire; chessboard line; execution; broker reveal behind the new president) → newsreel sv → cast list (Yoo Ah-in/Veteran's Cho Tae-oh rhyme; Park Sung-woong; Kim Hye-soo; Lee Sung-min special appearance) → 4-days-early row → verbatim closer (lip-red on women's mouths → on the church's white walls). Blocks: ls3/sv2/ckl1/pq1/pc1/char-intro1/sb7. Parity EXACT 46/44 — walker caught the dropped ？ on 「干嘛？」.

**Wrap & audits:** +72/+89 wrapper anchors; 1 illegal chr-peek inside ch220's pq stripped (§63.1 replay — post-wrap strip now standard); peek-in-blocks 0 bookwide; residuals = block rows/titles only; straight apostrophes 0; CJK/fullwidth/【】 0.

**Build & QA:** builder v54; epubcheck 0/0/0/0 first try. Battery vs V53 (corrected baselines): entries 378; chapters 220; spine/navPoints 224; nav chapter-li 220; images 127; cards/modals 87²; in-text 81; anchors 12,493 (12,333 +72 +88 — ch220's pq strip consumed one wrapped anchor, by law); ls 510; sv 234; pq 118; pc 96; chat 135; ckl 16; search-bar 1; naver-search 11; wb 35; mp 35; lb 31; parity ch215–220 all EXACT; PEEK 0. Delivered: Seoul_Starting_With_Debt_Collection__Version_54.epub, 22,740,647 B; V53 epub deleted after validation.

## §103 — V54 bugfix pass (user-reported chat bug + missing phone-call + real-photo law)

**USER BUG REPORT: "chat-container in chapter 219 — Receiver name & Sender name not properly implemented."** §52.7 full-bookwide scan (135 containers): the ONLY misattribution was ch219 — Eun-ah's label sat above Baek Si-on's sent bubble and the POV owner's label was missing entirely. REBUILT to the ch195 canonical shape: header "KakaoTalk · Baek Eun-ah", every bubble preceded by its own chat-name label, POV owner = `chat-name self` + chat-sent on the right. Bookwide label-vs-bubble audit now ships in the battery: MISATTRIBUTIONS 0.

**USER: missing .phone-call in ch220 (Choi Na-young ↔ Yoon Hye-ja).** The professor's call (「教授nim您好！」…「带一份简历来。」/ click-off) had shipped as plain dialogue-line paragraphs — converted to a `.phone-call` (pc-head "two o'clock · bring a résumé"; Na-young = pc-me, professor = pc-them; 「带一份简历来。」click as pc-note). Verbatim, parity held.

**SELF-CAUGHT during the deep scan: ch217's Rihanna 7 a.m. call was NEVER converted** — §101 recorded the fix as done; it was queued and dropped. ~45 paragraphs of plain dialogue carried a full two-party call. Converted to ONE `.phone-call` (pc-head "7:12 a.m. · the four-point framework"; Baek Si-on = pc-me, Rihanna = pc-them; setup lines restored outside the block; "The call ended." as final pc-note). §102's false record is hereby corrected. The UA call stays narrated (raw narrates it; no quoted dialogue — no block owed). In-block anchors unwrapped per §63.1 (−9; the outside setup line keeps its Rihanna anchor).

**USER LAW — real-person portraits (effective immediately):** "check the character is a real person or oc. If real then search the image from the web; if OC then AI generate." Kim Eun-sook is REAL → AI portrait replaced with a visually verified press photo (round glasses, Goblin mic, flanked by her DOTS/Goblin posters; 3-panel composite cropped 340x430+228+5 → 38,256 B q88 under the canonical filename). Song Hye-kyo lookalike results (2/4) rejected on identity check. Choi Na-young is OC → AI portrait stays.

**Recent-chapter block sweep (215–220):** ch218's jazz-piano playback stays plain narration — `.music-player` hasn't been used since ch97 and the track is unnamed in raw (no invented metadata); ch217 UA call narrated in raw = plain legal; ch215/216 have zero call scenes (0 pc correct); all other recent material verified block-shaped correctly (search-bar, ckl ×4, chat, pc ×5, sv ×10, pq ×3 across 215–220).

**Rebuild & battery (all green, 28/28):** epubcheck 0/0/0/0; pc 98; anchors 12,485; chat label misattributions 0 bookwide; kim-eunsook.jpg = real photo (38,256 B, JPEG magic verified in-zip); parity ch215–220 EXACT; PEEK 0; CJK/【】 0. Delivered: Seoul_Starting_With_Debt_Collection__Version_54.epub, 22,713,300 B (rebuilt in place; still the only epub on disk).

## §104 — V55 shipped (ch221–222 + Park Bo-gum real-photo card)

**Raw-first:** ch221 (836 ln, ？27/24 — double 「“你不是去接人了吗？艾琳呢？”经纪人问。」, triple 「经纪人：？？？」, 【】×1 Seung-wan's unsent draft) and ch222 (1154 ln, ？40/39 — double 「“下次？那你准备请我吃什么？”」, 【】×4 the four video titles) saved verbatim; guard green; cross-leak none.

**New card (88²):** Park Bo-gum — REAL person, REAL photo per §65.1 law: image_search → identity-verified 2015 Music Bank-era press shot (white-shirt smile era; a 3-panel Pinterest strip and a Soo-young/Ahn pairing rejected on identity), left panel cropped, 480×640, 61,292 B q88 under `park-bogum.jpg`. Card+modal registered; opf +1; FORMS +2 (Park Bo-gum / Bo-gum); in-text debut card in ch221 inserted POST-WRAP via the wrapped anchor (§62.2) with desc byte-identical to the intro page (§64.1 verified by diff).

**Missed-form repair:** "Park Soo-young" was carded (chr-joy) since ch215 but shipped ONE plain mention in ch215's closer; FORMS +2 (Park Soo-young / Soo-young → chr-joy) and ch215 re-wrapped (+1 anchor — now 45).

**ch221 ("The Consequences of Copying Your Sunbae"):** dorm 6:03 a.m. (the five named per canon: Irene/Seung-wan/Yeri/Soo-young/Seul-gi; Yeri's five-more-minutes) → van (script marks, ‘Return’ demo) → BodyArmor lightbox + full-side bus + blurred photo kept → flavor-honesty ("Baek sunbae's original, reduced-sugar") + math sv → KBS arrival, elevator split → staff chat overheard (‘The Producers’ premiere, Park Ji-eun script, IU/Milan) + amended-wish sv → MC prep (sailor suit; camera three) + Park Bo-gum in-text card → greenroom escape (teammate-care alibi) → studio glass ("my kid is going places") → lunch trays → WENDY intercom (producer's quoted line keeps "Wendy" per ch165 quoted-dialogue precedent; narration stays Seung-wan) → THE CORRIDOR: abstract dance, collar-grab, espresso sv → unsent-draft chat block (chat-name self Seung-wan; header labels it "the draft, never sent") + the three predicted reactions as plain prose → 「私人行程。」+ manager ??? (3-mark verbatim) → face-the-wall entrance → Kim Eun-sook declines to deduce → Song Hye-kyo bind as costed ckl (1.2B vs 200M, sixfold) → affordability sv (See You Again settlement math, 500k glosses) → Kim Ji-won pivot (sub-300M; seven-eight-years problem) → circles-to-squares → 90-degree bow + 辛苦了 register slip → corridor ambiguity sv (two readings as ？-carrying sv-lines) → the email (three swipes, unending list) → homework exchange → internal audit → "That just now — also the job?" checkmate → stomach growl + vending-machine-alibi sv → Eun-ah one-ring .phone-call ("Two.") → requirements list → her feet decide → elevator-door sv (two silhouettes; the short one pretending). Blocks: ls2/sv7/ckl1/chat1/pc1/char-intro1. Parity EXACT 27/24 — walker caught 3: the staff-chat ？ I flattened ("Probably, right?") and BOTH ambiguity sv-lines whose marks I'd dropped.

**ch222 ("Beauty Begins With Being Seen"):** Starex interior comedy (no bending required; buttons unpressed) → Yeouido pasta sv (2010 ‘Pasta’ wave exposition) → menu Italian (vocabulary = one menu + "thank you" + "the check"; no-cilantro/no-garlic for Bo-gum's sake; three sets for the bodyguards) → ISAC high-jump sv (the mat-rolled forescence) → "sunbae is such a bad boy" / vegetables → "I'll be checking" dread sv (chapter-three-video midnight) → carpaccio/al-dente → the flicked card ("Save it for high-jump shoes") → next-time pact → Seongsu: glasses-girl lens-first arrest → brand-diary slate as ckl (four 【】 titles converted) → 200k bonus + 10k-followers/1M rule, no ceiling → Vampire Cherry + First Peach → Na-young's layering idea → Eun-ah conscripted ("Manager Baek is the prettiest") → two-tone sv → pricing sv (19,900 psychology; 2,500 logistics; 30% channel skim; duo 34–35k) → "Let's do the brand name first" dodge → LAVUE whiteboard (La Vue; Love You easter egg) → thesis pq (title line verbatim) → LAVUE ONE whiteboard ckl (01 First Peach / 02 Vampire Cherry) → skeleton ckl (from Rihanna's freeload line to standing skeleton) → "We'll call it that" → cut-the-footage gag ("the walk-in part is shaky") → coached re-entry ("Boss — smile") → Music Bank wrap → dorm: Soo-young's WGM news, Seul-gi's district-office question, Yeri's screen, "What if it's Baek sunbae?" + Seung-wan's side-eye → wish-journal rebuke → Lee Min-ho pricing (Seul-gi, verbatim) → BodyArmor spot sv (whistle, vending machine, five colors, five one-liners, subtitle, logo) → living-room detonation → Melon chart as naver-search block (rows: See You Again #1 / OOPS #2 feat.; Ice Cream Cake #5 note; "first and second both have him") → three-hop joy-to-dread → Soo-young recruitment sv ("Teach me, unnie?"; the missing "me") → the living-room waltz disaster ("My lady. Are you ready?") → Seung-wan's stable-gift realization → final sv (ratings champion: not Lee Ji-eun, not Cha Tae-hyun — Soo-young, playing the hooligan husband). Blocks: ls3/sv7/ckl3/pq1/chart1. Parity EXACT 40/39 — walker caught the merged recruitment lines + dropped 「欧尼教我？」.

**Self-caught in QA (pre-commit):** (1) two CJK leaks in my own ch222 prose (性感/传播) — zero-tolerance scan caught; (2) ch222 chart built as unstyled `nav-block` — the book's styled chart class is `.naver-search` (nv-* children; CSS at stylesheet ~1768) — corrected; LAW: chart/ranked-list blocks use `.naver-search`, never a bare `nav-block`; (3) ch221 chat draft missing `chat-name self` on the POV owner — battery's bookwide chat audit caught it (1 → 0).

**Sandbox reset (§55.1 fired again):** /home/user/work symlink and the epubcheck/jdk4py toolchain were wiped mid-cycle; recreated symlink + `pip install --break-system-packages epubcheck jdk4py`; repo untouched. Builder v55; epubcheck 0/0/0/0. Battery ALL GREEN: entries 381; chapters 222; spine/navPoints 226; nav li 222; images 128; cards/modals 88²; in-text 82; anchors 12,762 (+131 +145 +1 ch215); ls 515; sv 248; pq 119; pc 99; chat 136; ckl 20; chart (naver-search) 12; search-bar 1; parity ch215–222 all EXACT; PEEK 0; chat misattributions 0; CJK/【】 0. Delivered: Seoul_Starting_With_Debt_Collection__Version_55.epub, 22,801,947 B; V54 epub deleted after validation.

## §105 — V56 shipped (ch223–224 + the bookwide short-name migration)

**Raw coverage (§ raws-first, ×16):** raw/chapter-223.txt (1005 ln) + raw/chapter-224.txt (447 ln) saved verbatim BEFORE any translation; 22 verbatim probes ALL PASS; ？-ledger ch223 48/48 (0 doubles/triples), ch224 25/25; Hangul 0; 【】 ch223 ×29 / ch224 ×0; no cross-leak.

**User directive executed — short-name migration:** bare "Seul-gi"→Seulgi ×42 and bare "Seung-wan"→Wendy ×78 via protected two-pass replace (full names masked first) across 15 xhtml (chapters 89, 109, 115, 161, 165, 181, 206, 208, 209, 210, 215, 216, 221, 222, characters.xhtml). Sulli/Yoona no-ops (bare forms were already 0 bookwide; Sulli canonical ×90). Full names preserved intact — Kang Seul-gi / Son Seung-wan plain counts 61/45 (the mid-cycle "59/37" was a mental-sum slip; verified identical before/after). Card display names (ci-seulgi "Kang Seul-gi", ci-wendy "Son Seung-wan") intentionally unchanged (full-name law). FORMS updated: 4 rows for chr-seulgi/chr-wendy (full + renamed short). **This revokes §66.3's "Seung-wan EXCLUSIVE" canon rule** — Wendy is the standard short form everywhere in narration too.

**Translation (ch223 "The "Yes" Worth Six Hundred Million" / ch224 "Just Asking His Taste, How Did We Land on Abs"):** ？-ledger built from raw BEFORE prose (line numbers listed, incl. 2×「嗯？」 and 「这位？」 that eye-count missed); drafted to ledger; two internal rhetorical questions initially flattened to statements (ch223 L402/L930) + one Melon comment with raw-terminal ？ flattened (L23) — all caught by the count diff and restored; ch223 48/48, ch224 25/25 EXACT from the built epub. Block ledger: ch223 plan ls3+ct1+chat11+ckl3+sv6 = shipped 3/1/11/3/6; ch224 plan ls2+ckl3+sv5 = shipped 2/3/5. 【】×29 ch223 landed as: Melon comment-thread ×4 (comment-item/comment-header/comment-floor, ch215 shape), KakaoTalk containers ×11 (Irene exchange ×7 incl. the deleted-draft container per §66.2 imagined-unsent law; LAVUE group ×4), A4 "Boom shaka laka boom" lyric line as plain printed-line paragraph (single quoted line, not a lyric sheet), LAVUE retraction narrated between containers per raw.

**Wrap & registration:** wrap_v29_anchors.py → ch223 +114 anchors (file total 115 incl. 1 manual), ch224 +80; display-row leak scan 0; XML parse OK ×2. Registered: content.opf manifest ch223/ch224 + spine itemrefs; toc.ncx np-227 (playOrder 227) / np-228 (playOrder 228); nav.xhtml li ×2. Builder cloned build_epub_v55.py → build_epub_v56.py (docstring + OUT only).

**V55 open defects folded in (all verified on artifact):** (1) "as forescen"→"as foreseen" (ch222 ISAC sv-header); (2) ch222 money per raw: "Two hundred thousand won"→"Two million won" (raw 两百万), "Two… two hundred thousand?"→"Two… two million?", "nineteen thousand nine hundred won"→"nineteen thousand won" (raw 一万九千), "Nineteen thousand nine hundred still lives"→"Nineteen thousand still lives", "at nineteen-nine"→"at nineteen"; (3) ch221 email scene: NO mail-block — raw L689–736 narrates the whole email (link→archive→folders) with zero structured content; plain paragraphs are faithful, defect closed as documented no-op; (4) OOPS/Oops case unify — law: EN mirrors raw at every occurrence (demo file names + production/planning refs = OOPS caps, ch202/204/207/218; released-title/chart/public refs = 'Oops', ch222 ×1 chart row + ch223 ×2 + ch224 ×3 flipped; bare "Oops" = lyric word). Bookwide after: ch222/223/224 contain zero OOPS caps.

**Build & checks:** v56 epub 22,822,651 B, 383 entries; epubcheck valid True, 0 messages (jdk4py java on PATH, §66.4 recipe); battery vs V55 (both artifacts, same method): entries +2, chapters 222→224, spine/navPoints 226→228, nav chapter li 224, anchors 12,762→12,957 (+195 = 115+80; the +114 wrapper figure excludes the 1 manual anchor — arithmetic closes), ls 515→520, sv 248→259, pq 115→115 (the "pq119" carried in memory was a method artifact — corrected canon is 115 under the pq-glyph pattern), pc 99=99, chat 136→147, ckl 20→26, comment 103→104, mail 8=8, naver-search 12=12, search-bar 1=1, images 128 disk = 128 declared, cards/modals 88/88, misattr 0 (88 modals mapped), Han 0 / fullwidth 0 / 【】0 (「」10=10 = ch78 §66.3 stage-name canon; battery range refined to Han+fullwidth-forms, CJK-punct brackets exempt).

**Deep scans:** ？mark-by-mark both chapters (per-line map, incl. the two mid-dialogue internal questions); typo battery no new defects (ds 16=16 pre-existing shipped; sbp 1=1 ch36 shipped; orphan-comma/dangling 0); §54.1 framed sweep 12=12 pre-existing shipped idiom (ch117 section-marker dashes, ch186/188/189 concert beats, ch193, ch199 certificate) — zero new; chat-coherence 1 flag = ch223 LAVUE container 2 (Eun-ah/Product girl/Market girl, all-received ≥2 parties) — INTENTIONAL: self Baek Si-on sends only the two photos (container 1) then goes silent ("did not stay to enjoy…"); raw-faithful silent-owner group-chat pattern, documented for the battery.

**Tooling traps logged (→ SKILL §67):** edit_file with old_text==new_text is a silent no-op and a "successful" fuzzy match can leave the target unchanged — always re-verify the artifact after edit_file and prefer python replace with assert-count==1 for surgical patches; quick-scan regexes must exclude self-closing `<p class="chat-clear"/>` (spanning matches inflated ch223 by 25 phantom double-space paragraphs) and must be paragraph-scoped for the framed sweep (cross-paragraph — matches).

**Delivered:** Seoul_Starting_With_Debt_Collection__Version_56.epub (keep-only-latest: V55 epub deleted on disk at ship).

## §106 — V57 shipped (ch225–226 + the golden-mouth deposit and the fakest fake husband)

**Raw coverage (§ raws-first):** raw/chapter-225.txt (354 ln) + raw/chapter-226.txt (1300 ln) saved verbatim BEFORE any translation; ？-ledger ch225 15/15 (0 doubles), ch226 72/72 (0 doubles); 【】 ch225 ×2 (L313 IU's KakaoTalk line, L349 bank SMS) / ch226 ×2 (L481 + L837 broadcast captions); Hangul 0; no cross-leak; 《》 inventory logged (The Producer / My Love from the Star / The Heirs / The Most Beautiful Moment in Life pt.1 / I NEED U / The Show / We Got Married / WGM / TIME).

**Deep-scan canon greps (all locked before drafting):** BTS canon from ch101/102/107 (Kim Nam-joon, Jeon Jung-kook, RM, Bangtan, BTS); the Eun-ah bag donor = Kim Jae-wook (ch171/176 canon); WGM = "We Got Married" (ch222); Park Ji-eun / 'My Love from the Star' / 'The Heirs' / Cha Tae-hyun / Gong Hyo-jin all prior canon (ch121–222); IU chat label "Lee Ji-eun" (ch121 shape); 「内」→"Ye." (ch221); Manager Baek (ch222/223); Mr. Baek (ch05/07/100); Oppa/Daebak/maknae/Aigoo existing registers. Yeri's full real name had NEVER appeared — 金艺琳 rendered "Kim Yerim" (first occurrence, documented); FORMS gained 2 rows: ("Bae Joo-hyun"→chr-irene, inserted before "Joo-hyun") and ("Kim Yerim"→chr-yeri, before "Yeri") — longest-first preserved; ast.parse OK. Yook Sung-jae introduced PLAIN (no card yet — WGM arc may recur; card pipeline offered next cycle, never silently churned). 金xx rendered "Kim XX" — faithful to the raw's own redaction of the male lead's name (documented decision, not an omission).

**Translation (ch225 "Jeong Han-teuk's Million-Won Golden Mouth" / ch226 "The Fake Husband Among Fake Husbands"):** ？-ledger built from raw BEFORE prose; statement-form traps held: the 4-line action playbook (要不要调查…/是否联系…/要不要反向铺…/是否帮…) ships as STATEMENTS inside the ckl (raw 。), "先问。" stays imperative, "那有。" stays statement, and the embedded "什么巧克力" quotation carries NO ？ (raw has none — eye-catch that failed, ledger caught). ch225 15/15, ch226 72/72 EXACT from the built epub, ？-line merge check clean (15 lines/15 marks, 72/72 — no line merges). One self-inflicted split caught pre-build: caption 1 drafted as "Really? No one at all?" (+1 mark vs raw's single ？) → "Really — no one at all?".

**Block ledger (deep-thinking conversion of 【】 + structured material):** ch225 plan ls2+ckl4+sv1+chat1+mail1 = shipped 2/4/1/1/1 — 【谢谢你的巧克力。】 → chat-container (self Lee Ji-eun, photo bubble + text bubble); 【KB国民银行】+入账+汇款人 → mail-block (mb-from "KB Kookmin Bank" / mb-subject "[Deposit notice]" / mb-body "Credit: 1,000,000 won. Remitter: Baek Si-on." — ch105 bank-mail precedent); media verdicts → ckl; doom loop → sv; suspect pool → ckl; action playbook → ckl; originals replay → ckl. ch226 plan ls4+ckl2+sv5 = shipped 4/2/5 — bonuses → ckl (recipients/contents/debt-exclusion/no-offset/gambling-clause, total 70M); prank design → ckl (4 steps + guardrails count); buzz-cut image → sv; PD signal chain → sv; 【真的没有吗？】+【威尼斯影帝首次用于婚恋诈骗】 → sv caption slates ×2 (no caption class exists in stylesheet — invented-class law held, existing sv reused); two-crises → sv.

**Wrap & registration:** +66 ch225 (file total 70 incl. 4 manual) / +196 ch226 (total 197 incl. 1 manual); display-row leaks 0; XML OK. Registered: opf ch225/ch226 manifest+spine; ncx np-229/np-230 (playOrder 230); nav li ×2 (chapter li 226). Builder cloned v56→build_epub_v57.py (docstring+OUT only).

**Build & checks:** 22,845,662 B, 385 entries; epubcheck valid True, 0 messages; battery vs V56 (artifact-to-artifact): entries +2, chapters 226, spine/navPoints 230, nav li 226, anchors 13,224 (+267 = 70+197), ls 526, sv 265, ckl 32, chat 148, mail 9, pq 115=115, pc 99=99, comment 104=104, naver-search 12=12, search-bar 1=1, images 128=128, cards/modals 88/88, misattr 0; Han 0 / fullwidth 0 / 【】0 / 「」10=10; Hangul 89 runs = characters.xhtml + glossary.xhtml front/back-matter canon, IDENTICAL across V56/V57 (glyph-count 241 both) — documented, not a regression.

**Deep scans:** typo battery 16 ds + 1 sbp = byte-identical to V56 shipped baseline (no new); §54.1 framed sweep 12=12; OOPS residual ch222–226 = 0; chat-coherence 1 flag = the documented V56 silent-owner LAVUE container (unchanged; V57's IU container is single-party self-sent = legal); ？mark-by-mark spot checks PASS ×10 (playbook statements, what-chocolate no-mark, SMS digits, caption marks, Ye?, Wendy soft call, full-name call, Kim XX, ckl items).

**Registers set (→ SKILL §68):** "Manager Baek-nim" (백매니저님); "Baek Si-on-ssi"; dorm full-name register — the raw's deliberate full names in the RV dorm sequence ship as full names (Bae Joo-hyun / Kang Seul-gi / Kim Yerim / Son Seung-wan / Park Soo-young), while Irene's soft short call 「胜完。」 ships as "Wendy." per the V56 short-name law (full-name call 「孙胜完。」 stays "Son Seung-wan."); "glass-wiping auntie laugh"; "braised egg"; money in SMS = ASCII digits (1,000,000 won), prose = words (ten million won).

**Delivered:** Seoul_Starting_With_Debt_Collection__Version_57.epub (V56 epub deleted on disk at ship).

## §107 — V58 shipped (ch227–228 + the wealth code and the six-billion-won lunch)

**Raw coverage (§ raws-first):** raw/chapter-227.txt (736 ln) + raw/chapter-228.txt (868 ln) saved verbatim BEFORE any translation (one self-caught drift 座椅里→座椅上 fixed pre-probe; all 15 verbatim probes incl. the &lt;entities&gt; line PASS); ？-ledger ch227 40 marks / 39 lines (L124 double: 压力更大？…更强？), ch228 25 marks / 24 lines (L819 double: 谈什么？续约？); 【】 ch227 ×3 (L524/L544 chat, L687 bank) / ch228 ×0; Hangul 0; hashtags ×4; 《》 inventory (除三害 NEW, 绿头苍蝇, 思悼, 局内人, 制作人, CeCi, Way Down We Go, 太阳的后裔).

**Deep-scan canon greps:** Sado = 'Sado' (ch110, Crown Prince Sado / Lee Joon-ik); 绿头苍蝇 = 'Green Bottle Fly' (ch123 search-bar canon); 《局内人》 = 'Inside Men'; Kim Hye-soo / Yoo Ah-in / Lee Sung-min / Lee Byung-hun prior canon; Park Seong-woong, Jo Woo-jin, Kim Seon-young NEW (plain, documented); Chung Eui-sun plain (ch142 precedent); Seo Eun-ju = attorney (ch100–102); Cindy ×80 (IU's Producer role); 臭桃子 = 'Brat of a Peach' (ch136 ×2 canon); 部队锅 = budae-jjigae; 阿西 = "Aish"; Manager/Chairman-nim registers. **除三害 had NO prior rendering — titled 'The Three Evils' (decision documented, used consistently ×2 chapters).** 金艺琳-class checks: no new FORMS rows needed (BTS/IU/cousins all carded).

**Translation (ch227 "Jeong Han-teuk Discovers the Wealth Code" / ch228 "The Six-Billion-Won Lunch"):** ？-ledger BEFORE prose; BOTH double-mark lines shipped as single lines with both marks; ONE raw-line merge caught by the new line-count diff post-build (ch227: L204 现实里动手呢？ split into 2 EN marks + L606 该不会是你送的吧？ demoted to a period — both fixed; 40/40 marks + 39/39 lines EXACT from rebuilt epub; ch228 25/25 + 24/24 EXACT). One stray CJK (正式) caught in self-review pre-wrap; one invented pq wrapper class removed (§61 law); invented supplier dialogue in the pc replaced with raw-narration lines (no fabricated speech); 嫂子 relation corrected (sister-in-law = the director's, i.e. Baek Si-on's mother).

**Block ledger (deep-thinking conversion):** ch227 plan ls2+ckl2+sv4+nd1+pq2+pc1+chat2+mail1 = shipped EXACT — media verdicts→ckl, pre-written headlines→sv ×3-slot, doom-loop→sv (V56), Yoo Ah-in headline→sv, Naver entertainment feed→news-digest ×4 rows (incl. the 《……》 ellipsis row; ch171 multi-row precedent), manager's theorem→pq, wealth code→pq, catering order→.phone-call (pc-me only — raw quotes Han-teuk alone; supplier turns kept as narration INSIDE the block), IU support photos+thanks→chat (received, self=BS), IU's reply→chat (self), hashtags→sv push-column (#BaekSiOn* compact tags), 5-way win→sv, bank SMS→mail-block (KB, digits). ch228 plan ls2+ckl6+sv1 = shipped EXACT — cigarette defense→ckl, camps→ckl (Jo Woo-jin neutral, budae-jjigae count), table→ckl (gujeolpan/hanwoo/soy-marinated crab), Genesis offer→ckl (3yr × 2B, scope, exclusions, 0.1% uncapped), incentive binding→ckl, rollout→ckl (CeCi cover + LG Times Square ≥1 week), playbook→sv (Toyota→Lexus; the raw's “N” badge mirrored verbatim).

**Wrap & registration:** +69 ch227 (file total 71 incl. 2 manual) / +89 ch228 (total 90); display-row leaks 0. **ch227 XML FAIL on first parse — the phone-call div was never closed (caught by the minidom re-parse, fixed, OK).** Registered: opf ch227/ch228 manifest+spine; ncx np-231/np-232 (playOrder 232); nav li ×2 (chapter li 228). Builder v57→build_epub_v58.py (docstring+OUT).

**Build & checks:** 22,869,005 B, 387 entries; epubcheck valid True, 0 messages (after rebuild); battery vs V57: entries +2, chapters 228, spine/navPoints 232, nav li 228, anchors 13,385 (+161 = 71+90), ls 530, sv 270, pq 117 (+2), pc 100 (+1), chat 150 (+2), ckl 40 (+8), mail 10 (+1), nd 70 (+1), pq115→117, comment/naver/sb 104/12/1 flat, images 128=128, cards/modals 88/88, misattr 0, Han 0 / fullwidth 0 / 【】0 / 「」10=10, front-matter Hangul 241=241.

**Deep scans:** typo battery 16 ds + 1 sbp = V57 baseline (no new); framed sweep 12=12; OOPS residual ch222–228 = 0; chat flags 1 = the documented V56 LAVUE silent-owner (ch227's received-container is single-party = legal); mark-by-mark spot checks 20/20 PASS; Kim XX redaction intact (ch225).

**Delivered:** Seoul_Starting_With_Debt_Collection__Version_58.epub (V57 epub deleted on disk at ship).

## §108 — V59 shipped (ch229–230 + the pq-wrapper bug fixed and the Red Bebe set-visit day)

**User-flagged bug FIXED (owed since V58):** the ch227 `<p class="pq-glyph">❝</p>` bare trios (agent's theorem + wealth code) are now wrapped in `<div class="pullquote">` matching the ch215/ch222 canon shape. Deep scan first: 108 canon sites wrapped; 7 "bare" hits (ch09/10/13/59/119/121/128) are the LEGAL ch09-era span idiom already inside pullquote divs (untouched per directive); only ch227's 2 trios were true bugs. First repair regex found 0 (assumed blank lines + anchor-free middle `<p>`; actual shapes: no blank lines, middle `<p>` carries chr-inline anchors) — tolerant pattern `^(\s*)(<p class="pq-glyph">❝</p>)\n( *<p>.*?</p>)\n( *<p class="pq-cite">.*?</p>)` matched ×2, wrapped, minidom OK. Book-wide bare-pq battery = 0 (ch13 legacy flagged by the naive last-`</div>` scanner = false positive, verified by eye).

**Raw coverage (§ raws-first):** raw/chapter-229.txt (1122 ln) + raw/chapter-230.txt (832 ln) saved verbatim BEFORE translation; 31 verbatim probes PASS. ？-ledger: ch229 35 marks / 35 lines / 0 doubles; ch230 23 marks / 23 lines / 0 doubles. 【】 ×0 both; Hangul 0 both; hashtags ×0. ch229's literal `-----------------` divider → scene-break ❧ (ch219 precedent; dash-dividers exist in raws 200/219/229, always converted).

**Canon greps (raws ungrepped at save → swept this cycle):** Kim Eun-sook (金恩淑, writer) NEW plain; Baek Ji-young (白智英) NEW plain + 《像中枪一样》 = 'Like Being Hit by a Bullet' (first rendering); 《学校2015》 = 'School 2015' (×2 prior); Kim Eun-sook dramas titled: 'Lovers in Paris' / 'Secret Garden' / 'A Gentleman's Dignity' (all first renderings, documented); 《爱乐之城》 = 'La La Land' (×6 prior); 私生饭 = "sasaeng" (first rendering, k-culture register); f(x) ×48 prior; League of Legends Season 4 (ch218 precedent); 室长 = "Manager Baek" (ch222/223 register); 莱德贝贝 = the guard's mangle rendered "Red-eh Beh-beh" (title 'The Red Bebe Set-Visit Day'); 傻帽 = SM; 葡萄口味/西瓜 = grape flavor / watermelon (ties to the ch223 watermelon-princess beat); 降维打击 = "a dimensional mismatch"; 姐夫 = "brother-in-law" (ch226 register); LoL S4 group photo with three people digitally purged = sv block. Yook Sung-jae absent (card offer stays open).

**Translation (ch229 "The Red Bebe Set-Visit Day" / ch230 "A Loophole Is Also Part of the Rules"):** ？-ledger BEFORE prose; both EXACT from built epub on first build (35/35 + 23/23 — zero merges needed this cycle). Self-review caught 3 defects pre-wrap (ch229: week-tally block + prose duplication, invented "sixty-million... six-hundred-million" verbal stumble, wrong jargon "two-hots"; fixed via assert==1 sw()); ch230 same-class catches (V-photo dedup, sv-note/prose duplication, 降维打击 phrasing). Title caps fixed (the Red Bebe → The Red Bebe).

**Block ledger (deep-thinking conversion):** ch229 ls4+ckl2+sv2+pq1 — dorm interrogation / kitchen gimbap / garage night shoot / set entrance; light drill→ckl, sound drill→ckl (PAK! slate cracks; jogging-lap cure), guard's week tally→sv (ten groups: 8 cousins + 2 fiancées), purged group photo→sv, Jo Woo-jin's night-run verdict→pq (properly inside div.pullquote). ch230 ls1+ckl1+sv2+pq1 — captain-seat van; favors list→ckl (BodyArmor, solo lunch, LAVUE, DotS cameo, CeCi cover, Minion harmonies, retaliatory dance homework), changed title→sv (main vocal → waste-sorting staff), locked V-photo→sv (her V = un-hideable delight, his V = un-bothered indulgence), loophole ruling→pq. TheOST-bribe economics (2M-won trucks vs gimbap sincerity), four-lunchboxes-for-three puzzle, blinking-contest rules-lawyering (no countdown law / rule change mid-match / referee Son Seung-wan), stealth selfie + seat-gap phone.

**Wrap & registration:** ch229 manual ×7 (written with anchors); wrap pass with EXPLICIT argv (the script defaults to ch171/172 and silently wraps nothing new otherwise — near-miss this cycle): +145 ch229 / +154 ch230 → book total 13,691. Display-row leaks 0 (sv/ckl rows anchor-exempt by design; the 2 remaining "bare" names sit in sv rows = legal). Minidom re-parse all chapters OK. Registered: opf manifest+spine ch229/ch230 (230 items = spine = 230); ncx np-233/np-234 (max playOrder 234); nav li ×2 (230 chapter li).

**Build & checks:** build_epub_v59.py (docstring+OUT cloned); 22,894,421 B, 389 entries; epubcheck valid True, 0 messages. **Decisive battery V58↔V59 sha1 diff: exactly 6 changed entries** (opf/ncx/nav + ch227 pq repair + 2 new chapters), everything else byte-identical — prior book-wide counts carry flat (chat 150 / pc 100 / mail 10 / comment-104-class / naver-search 12 / search-bar 1 / images 128 / cards-modals 88/88 / Hangul 241 front-matter). New totals: chapters 230, spine/navPoints 234, nav li 230, anchors 13,691, ls 535, sv 274, pq 119, ckl 43. Han 0 / fullwidth 0 / 【】0 in chapters. ？ from built epub: ch227 40/39, ch228 25/24 (unchanged by pq wrap), ch229 35/35, ch230 23/23 — ALL EXACT, merge-free.

**Deep scans:** spot checks 20/20 PASS (incl. 莱德贝贝 mangle ×3, grape→watermelon, Baek Ji-young OST line, four lunchboxes, front-camera excuse, seat-gap maturity); misattr sweep clean (brother-in-law research partner = Soo-young, verified vs raw L275); doubled-word scan clean (La La Land = false positive); OOPS ch229 ×1 = the canon ‘Oops’ magazine recognition beat (legal); framed/peek parity 152=152 / 154=154.

**Delivered:** Seoul_Starting_With_Debt_Collection__Version_59.epub (V58 epub deleted on disk at ship).

## §109 — V60 shipped (ch231–232 + the style-block anchor purge + the ls spacing fix)

**User law #1 ENFORCED (no anchors/hover previews inside ANY style block):** deep scan found THREE leak shapes across the eras — (a) full-shape peek anchors inside ch227's pq ×2 + phone-call narration ×4, ch200 nd ×1, ch147/149 ×1 each (9 stripped); (b) LEGACY no-peek anchors (`<a class="chr-inline" href="...">Name</a>`, no chr-peek span) inside pq/sv/wardrobe blocks in ch125/167/201/205/206/209/211/212/213/214/215/216 (43 stripped — the first strip missed them because its regex REQUIRED the peek span); (c) the wrap pass leaked 18 into V60 drafts (classless in-block narration + pq body are classless/dialy paragraphs the script didn't guard). All 70 stripped (strip wrapper, keep plain name); book-wide battery anchors-in-blocks = 0, chr-peek parity intact (13,632). **wrap_v29_anchors.py hardened** with a block-region guard (any paragraph starting inside a non-page-wrapper div is returned untouched).

**User law #2 FIXED (ls overlap):** root cause = `.location-stamp` flex `gap: 0.16rem` — some EPUB renderers drop flex-gap entirely, so ls-date/ls-time-pill/ls-place collided. Explicit margins added (.ls-date mb 0.34rem, .ls-time 0.36/0.38rem, .ls-place mt 0.34rem) — spacing no longer depends on flex-gap.

**Raw coverage (§ raws-first):** raw/chapter-231.txt (647 ln) + raw/chapter-232.txt (1029 ln) saved verbatim BEFORE any translation; 37+21 verbatim probes PASS (incl. &amp; entity, 2014——2015 double dash, 真理xi, 星材xi). ？-ledger: ch231 37/37, 0 doubles; ch232 46 marks / 45 lines / 1 double (L827 今天录制怎么样？…相处得好吗？). 【】 ch231 ×0 / ch232 ×13; Hangul 0; hashtags 0.

**Canon greps:** 郑在俊 = Jung Jae-joon (CARDED, chr-jung-jae-joon, ×388 corpus!) — Irene's cover story; 黄秀雅 = Hwang Soo-ah (CARDED, chr-hwang-sooah, ×45) — the LAVUE-adjacent commercial director; 'We Got Married' established (curly quotes, ch222/226); Oppa ×158; Joohyun-unnie (fused, ×4 precedent) → Seulgi-unnie coined on the same pattern; X-ssi register (Jin-ri-ssi, Sung-jae-ssi); Yook Sung-jae ships PLAIN (×26 prior — plain-by-canon like Jo Woo-jin); COSMAX (new, OEM register); CJ Korea Express (era-correct 2015 name); Estée Lauder ×3 / Laneige ×2 / Song Hye-kyo ×4 / Under Armour ×83 / Vanessa ×9 / Bluehole ×17 all prior; 'Love Yourself' / 'See You Again' / 'Legend' / 'Jumpshot' canon; 圣母 = "the Holy Mother"; 赵宇镇 → Jo Woo-jin (character scenes ship actor name, ch229 convention); Kim Hye-soo / Yoo Ah-in plain (uncarded, ch227 precedent); Melon daily/weekly chart; 1：0 → 1-0.

**Translation (ch231 "Corny Pickup Lines Are Also Accepted" / ch232 "The Invisible Ex Invades the Marriage"):** ？-ledger BEFORE prose; ch231 EXACT 37/37 after self-catch ("who didn’t." → "?"); ch232 self-caught: stray CJK 纠缠 fragment in a tangle sentence, a copy-paste WRONG ANCHOR (Jo Woo-jin wrapped in chr-choi-jinri — stripped to plain), two demoted ？ restored ("You just asked me that?" / "So you have met plenty of times in private?"), and post-build polish ("She had had no intention" → "She had not meant to"; keep one legal-but-ugly catch). FINAL from built epub: 37/37 + 46/45 EXACT incl. the Sung-jae-ssi double as one line.

**Block ledger (deep-thinking conversion, MORE blocks — no compromise):** ch231 ls3+ckl3+pq1+pc1 — three-doors massacre→ckl, Bluehole pre-run routes→ckl, "the evil speaks for itself"→pq, casting math (Song Hye-kyo tier 2.4B vs Bae Joo-hyun 3yr/600M + Laneige clause)→ckl, Eun-ah production call→.phone-call TWO-SIDED (pc-me + **pc-them FIRST USE** — the CSS class existed unused since the block's birth; pc-head names both parties PLAIN). ch232 ls4+nd1+sv3+comment1+mail1+pc1+ckl2+pq2 — Naver hot-search ×4→nd, WGM edit (pink caption, system-husband caption, bubbles/FX/freeze)→sv, netizen roast ×4 + 【……】→comment-list (floor N · up users), crew delay SMS→mail-block [Crew notice] (digits), wanted poster→sv, Kobe farewell call→pc two-sided (retirement, 'See You Again' 4th week, BodyArmor, Rockets), vocal audit (verse one voice / four processed chorus lines / IRENE by elimination)→ckl, Holy Mother brochure→ckl, "romance dies by call sheet"→pq, Seulgi's bear-cookie unified theory→closing pq.

**Registration & build:** opf manifest+spine ch231/ch232 (232 = spine); ncx np-235/np-236 (max playOrder 236); nav li 232. build_epub_v60.py (docstring+OUT cloned). **22,917,611 B, 391 entries; epubcheck valid True, 0 messages.** Decisive battery V59↔V60 sha1 diff: 22 changed entries — EXACTLY opf/ncx/nav + stylesheet.css + 16 strip-touched chapters + 2 new chapters; everything else byte-identical (prior ？ counts and block counts carry flat). New baselines: anchors 13,790 (+99 net = −70 stripped +9 manual +160 wrap −18 leak-strip), peek 13,632, ls 542, sv 277, pq 122, ckl 48, pc 102, mail 11; anchors-in-blocks 0; Han/fw/【】 0 in chapters.

**Deep scans:** spot checks 32/32 PASS (Jin-ri-ssi, COSMAX, CJ Korea Express, Hwang Soo-ah, Joohyun-unnie, Miss Choi, Oppa’s heart, Holy Mother, IRENE credit, Sung-jae-ssi, Art required it, Seulgi-unnie, Rockets, Vanessa…); misattr clean (Jo Woo-jin ×6 plain, Kim Hye-soo ×1 plain, Yook Sung-jae ×10 plain, "朴秀荣。" full-name dread kept); doubled-word scan clean after the "had had" polish; OOPS ch232 ×2 = WGM music cue + sv row (canon); pre-package ？ re-audit from artifact EXACT.

**Delivered:** Seoul_Starting_With_Debt_Collection__Version_60.epub (V59 epub deleted on disk at ship).

## §110 — V61 shipped (ch233–234 + the comment-block fix + the tv-screen block built and applied book-wide)

**User-flagged bug #1 ROOT-CAUGHT (the "missing" comment block):** the block existed in ch232 but shipped with an INVENTED container class `comment-list` — zero CSS (the working container since ch93 is `comment-thread`) — so it rendered unstyled. Renamed; book-wide class audit (every class in every chapter checked against stylesheet.css) confirms it was the ONLY invented class in the book. New battery: comment-list residue = 0.

**User-flagged bug #2 (the on-screen ideal-type dialogue):** Park Soo-young's three mission-card lines ("Gentle..." / "Taller..." / "Someone who feels safe...") were plain dialogue-lines outside the edit block — now the lead segment of a NEW sv block "The mission card, as read to camera" (with the pink-caption sv-note). ？-ledger untouched (46/45 holds).

**NEW BLOCK (user-mandated): `.tv-screen`** — television / studio-monitor / video-wall / played-back-video content, styled & colorful: rounded dark frame with bezel shadow, gradient channel bar with live dot (tv-channel), rows (tv-line), gold italic quotes (tv-quote), note row (tv-note). Applied book-wide: ch232 half-basement TV news (converted; anchors stripped from merged tv-lines per §71.1), ch233 ×3 (the LAVUE ad premiere on the office monitor, the So Beauty upload, the startup-diary episode), ch234 ×2 (the LG showroom display wall, the 'Sado' teaser) = **6 book-wide**. Register: tv-screen = broadcast/video playback watched on any screen; screen-view stays phone/app UI.

**Raw coverage (§ raws-first):** raw/chapter-233.txt (885 ln) + raw/chapter-234.txt (834 ln) saved verbatim BEFORE translation; 25+30 probes PASS. ？-ledger: ch233 49 marks / 46 lines / 3 doubles (the So Beauty user questions); ch234 30 marks / 29 lines / 1 double (架空历史呢？让他真的杀进去？). 【】 ch233 ×41 / ch234 ×17 — all routed to blocks (whiteboard ckl, sold-out sv, fan/user/vlog/roast comment-threads ×4 in 233, nd ×1, mail ×2, chat ×2 in 234, push nd); Hangul 0.

**Translation (ch233 "The Boss Is Out, So the Company Sold Out" / ch234 "Legends Never Fall"):** ？-ledger BEFORE prose. Self-review caught: 2 stray CJK fragments (想象/敷衍), a WRONG ANCHOR (Yoon So-yeon wrapped in chr-baek-eunah — she ships plain), 6 over-count ？ (raw single-？ lines split into two EN questions), 2 missing ？ (放吗？ folded into a ckl; 公关件？ narration), the fan-tax headline given an extra ？, one merged double (唇色偏深), one over-？ chat line (hm? → hm.) — final from built epub: **49/46 + 30/29 EXACT**. Title decision: 传奇永不陨落 = **'Legends Never Fall'** (in-universe S5 song concept; documented).

**Blocks:** ch233 ls4+tv3+ckl3+sv3+comment-thread4+nd1+mail2+pc1+pq2 (whiteboard ckl, media-plan ckl 排面vs订单, restock-math ckl, launch dashboard sv incl. SOLD OUT popup + title card, fan-receipts/forum/user-questions/vlog-roast comment-threads, Eun-ah two-sided call incl. the 27→12 'Sado' promo table, sales-is-ad pq, find-Mom closing pq). ch234 ls2+ckl2+tv2+sv1+nd1+chat2+pc1+pq2 (campaign calendar ckl, Riot term sheet ckl, Sado teaser tv-screen with Yeongjo quotes, LG wall tv-screen, bus-stop poster sv, KES chat + Irene premiere chat, Scooter call pc-them, legends thesis + Staples-fantasy pq).

**Canon greps:** Jung Jae-joon / Hwang Soo-ah / Choi Na-young / Seo Eun-ju / Lee Mi-kyung / Song Kang-ho / Lee Joon-ik / Kim Eun-sook all carded (prose anchors only); Kim Ji-won / Yoon Myung-joo / Moon Geun-young / Lee Sang-hyeok / Lee Eung-bok / Yoon So-yeon ship PLAIN (×5/×6/×10/×12/×17 prior); shades No. 01 First-Love Peach / No. 02 Vampire Cherry (register locked); 'Minions' (film), 'Infinite Challenge' + 'Weekly Idol' (first renderings), 'Running Man' ×6; "philanthropist Baek"; Seongsu-dong / Mapo; prose-money words (seven hundred sixteen million won; three point three billion won); 'Sado' teaser date Aug 3 mirrored beside the plan's "around August 2" (verbatim law).

**epubcheck catch (pre-ship):** first build FAILED RSC-007/012 — I had anchored Lee Sang-hyeok (chr-faker/faker.jpg) and Lee Eung-bok (chr-lee-eungbok/lee-eungbok.jpg) believing them carded; no such cards/images exist and prior usage is bare ×12/×17. Stripped to plain; VALID True, 0 messages. Lesson logged (§72.3): grep the card AND the image file before anchoring.

**Build & checks:** build_epub_v61.py; **22,942,547 B, 393 entries; epubcheck valid True, 0 messages.** Battery V60↔V61 sha1: exactly 5 changed (opf/ncx/nav + stylesheet.css + ch232 fixes) + 2 new chapters — everything else byte-identical. Baselines (re-derived from the artifact): chapters 234, spine/navPoints 238 (np-238), nav li 234, anchors 13,896 / peek 13,738, anchors-in-blocks 0, tv-screen 6, ls 548, sv 284, pq 127, ckl 53, pc 104, mail 13, comment-item 623, Han/fw/【】 0.

**Deep scans:** spot checks 30/30 (the one "miss" was a mislabeled probe — philanthropist Baek is ch234, verified present); ？ from artifact: ch231 37/37, ch232 46/45, ch233 49/46, ch234 30/29 ALL EXACT; doubled-words 0; OOPS 0 in new chapters; misattr guards clean (Kim Ji-won ×7 / Song Kang-ho ×4 / Lee Joon-ik ×9 / Moon Geun-young ×1); comment-list residue 0.

**Delivered:** Seoul_Starting_With_Debt_Collection__Version_61.epub (V60 epub deleted on disk at ship).

## §111 — V62 shipped (ch235–236 + the 'Legends Never Die' correction)

**OWED REPAIR APPLIED (raw-verbatim law §69.6):** V61 had coined 'Legends Never Fall' from 传奇永不陨落 — but the RAW itself writes the song in English: 《Legends Never Die》(传奇永不陨落 is its gloss). All 5 sites corrected: ch234 title + h1 + pq-cite, toc.ncx np-238 label, nav li, SETUP checklist. ？ ledger untouched (30/29 holds). Worklog §110 records what V61 shipped (history kept); this is the canonical name going forward.

**Raw coverage (§ raws-first):** raw/chapter-235.txt (661 ln) + raw/chapter-236.txt (402 ln) saved verbatim BEFORE translation; 27+21 probes PASS (one probe-string typo self-caught: the 替他 line has 这段时间， before the quote). ？-ledger: ch235 22/22, 0 doubles; ch236 11/11, 0 doubles. 【】 ch235 ×8 → all routed to blocks (counter signs sv, social post comment, official statement sv, whiteboard caption sv); ch236 ×0; Hangul 0.

**Canon greps:** 尹惠子教授 = **Yoon Hye-ja — the CARDED chr-yoon-hyeja** (deep-scan confirmed she IS the professor: prior corpus carries "Yoon Hye-ja ↔ Choi Na-young · voice", "Yoon Hye-ja's empty house") — anchored in prose ×2. 金智敏 = **Kim Ji-min, plain** (uncarded; must NEVER shorten — "Ji-min" alone collides with carded Park Ji-min ×12; zero leakage verified). 《打上花火》 = 'Uchiage Hanabi' (prior rendering reused); 无限挑战歌谣祭 = "this year's 'Infinite Challenge' music festival"; 圣母/Kim Hye-soo plain (ch227 precedent); Goo Hara / Jung Jae-joon / Jeong Han-teuk / Lee Ji-eun / Choi Na-young / Bae Joo-hyun / Baek Eun-ah carded (prose-only).

**Translation (ch235 "A Hundred Thousand Units Isn't Even a Snack" / ch236 "Chocolate in Hand, Off to Drag Someone Into Overtime"):** ？-ledger BEFORE prose. Self-review: ch235 had ONE extra ？ (an invented comment-floor question the raw's narration doesn't carry) → reworded to statements, 22/22 EXACT; ch236 EXACT 11/11 on first pass. Draft-slip caught by battery: 3 anchors hand-written inside blocks (pc narration ×2, tv-screen ×1) → stripped per §71.1 (the wrap script correctly declined to add them; the slip was mine at draft time).

**Block ledger (deep-thinking conversion, MORE blocks):** ch235 ls3+tv1+ckl2+pc1+sv3+nd1+comment1+pq2 — counter ad loop→tv-screen, training→ckl, checkout quartet (Scan/Pay/Bag/Next)→ckl, Ji-min↔Na-young restock call→pc (two-sided; multi-store cascade narrated in-block), noon dashboard→sv (30,000→21,000→SOLD OUT under ten minutes), counter signs→sv, empty-counter post→comment-thread (tester-stick preorder chorus), 6 media headlines→nd (incl. the 《……》 row), official statement→sv (with the ……LAVUE全体员工敬上 note row), cake→sv (four sugar-cream figurines), fancam-nuclear threat→pq, Eun-ah's battlefield-fee rate card→closing pq. ch236 ls2+tv1+sv1+ckl2+pc2+pq2 — church massacre take→tv-screen (Kim Hye-soo's last lines as tv-quotes; "Cut." as note), schedule page→sv (two free days), Jae-joon call→pc (Japan, 'Uchiage Hanabi', templates on the main machine), demo versions v1/v2/v3→ckl, one-man-shuttle session→ckl, Han-teuk call→pc (music-festival context, "Tell her that yourself", booking a head-on collision in-block), "legends don't kneel"→pq, chorus thesis (lost/aged/dropped/name stays)→pq.

**Registration & build:** opf manifest+spine ch235/ch236 (236 = spine); ncx np-239/np-240 (max playOrder 240); nav li 236. build_epub_v62.py cloned. **22,958,352 B, 395 entries; epubcheck valid True, 0 messages.** Battery V61↔V62 sha1: exactly 4 changed (opf/ncx/nav + ch234 Legends fix) + 2 new chapters — everything else byte-identical. New baselines (from artifact): chapters 236, spine/navPoints 240, nav li 236, anchors 13,973 / peek 13,815, anchors-in-blocks 0, tv-screen 8, ls 552, sv 290, pq 132, ckl 56, pc 107, mail 13, comment-item 626, Han/fw/【】 0.

**Deep scans:** spot checks 30/31 (1 probe-string case typo — content verified present); ？ from artifact ALL EXACT (233 49/46, 234 30/29, 235 22/22, 236 11/11); doubled-words 0; OOPS 0 in new chapters; misattr guards clean (Kim Hye-soo ×3, Kim Ji-min ×10 with zero Park Ji-min leakage, Yoon Hye-ja ×2, Goo Hara ×1).

**Delivered:** Seoul_Starting_With_Debt_Collection__Version_62.epub (V61 epub deleted on disk at ship).

## §112 — V63 shipped (ch237–238: the chocolate overtime + the OST confession)

**Raw coverage (§ raws-first):** raw/chapter-237.txt (1,154 ln) + raw/chapter-238.txt (806 ln) saved verbatim BEFORE translation; 41+43 probes PASS. ？-ledger (both widths — ch237's "Legends?" carries a HALF-WIDTH ?): ch237 **82 marks / 81 lines / 1 double** ("一首歌？？"); ch238 **45/45/0**. 【】 ch237 ×6 → all into blocks (sent-success + 4 memo lines + phone-assist row); ch238 ×0; Hangul 0.

**Ledger discipline paid again:** first-pass ch237 = 78/77 — 5 marks lost by folding raw dialogue into blocks ($3M pair, twenty-six-songs pair) plus a merged narration double (lines 33/34) and ONE invented ？ ("why had he come here to be scolded?" — the raw's 何必 line has NO mark) → all restored/corrected to 82/81 EXACT; ch238 first pass 45/44 → one merged narration pair (insert-song lines 12/13) split → 45/45 EXACT. NEW LAW (§74.1): fold content into blocks ONLY when the marks survive verbatim elsewhere; every block-conversion re-runs the full line-by-line ledger.

**Canon locked:** Park Myung-soo PLAIN (new; "Myung-soo sunbae" in dialogue); 'Leon' stage concept = 'Léon: The Professional', Mathilda/Léon; 'Meet Me on Friday' + 'Besides Spring, Love, and Cherry Blossoms' (both shipped renderings reused EXACTLY); 'Descendants of the Sun' (×9 precedent); Kim Tae-yeon + Baek Ji-young PLAIN (rate-card ladder); Gibson J-160E / John Lennon guitar (shipped canon); $2.4M ≈ 2.7B won, $3M > 3.3B won (prose money in words, block money in digits); 'Wolf of Wall Street'; KBS Cool FM 'Park Myung-soo's Radio Show'; Showbox; Choi Jin-ri full ×2 (bare Jin-ri exempt); Yoo Si-jin / Kang Mo-yeon / Cindy / Baek Seung-chan / Park Ji-eun PLAIN (all in-verse precedents); 'Good Days' restaurant (shipped name; her spoken "At 'Good Days?'" keeps the raw's mark); Kim Eun-sook carded-anchored; Song Kang-ho carded-anchored (radio-refusal rationale); sung lines → music-player mp-lyric ONLY ("Legends never die..." / "When the world is calling you..." — booth feed card); handwritten page → lyric-block (lb-header + plain rows + lb-note) ×3 total (first sheet / chorus as finished / chorus as reread).

**Translation (ch237 "One Guitar for Twenty-Six Meetings" / ch238 "The Confession Hidden in the OST"):** chocolate-wages banter → vending-machine stand-off; 'Leon' disagreement → ckl as tabled; "between has-a-point and is-right stands a whole song"; booth session (track map ckl, takes ckl, booth-feed music-player, her therapeutic fault-finding); delivery sv (To Riot, cc Scooter, "Sent successfully"); rate-card ckl ($3M page sv, prose conversion "past three point three billion won"); guitar math sv ($2,400,000 ≈ 2.7B won → twenty-six songs + half → fifty-two evenings); memo sv (Lennon Guitar Production Debt / Total: 26 / Completed: S5 assistance — one song / Remaining: 25) + amended sv ("Phone-movement assistance: pending."); the flick that drops the phone, the Wolf-of-Wall Street bidding (0.1 → 0.3), the scratch = "emotional damages — I have feelings"; gangster-police-soldier reveal; props-law pq + standing-appointment pq + closing melody pq. ch238: Jin-ri confession pq ("filed under a drama title"); lyric-block page (six lines, ink bleeding, the glance that never happened); chorus lyric-blocks; tug-of-war over the page → stationery pq; 'Say It! What Are You Doing?' named & interrogated; freebie/buy-one-get-one + the quote screenshot sent to Kim Eun-sook; second song written at the keyboard (writing-split ckl + concept ckl → 'Everytime'); labor-law strike (8 hours, union next); 'Good Days?' dinner plan; Han-teuk call pc (crew already downstairs — "the city gate had fallen"); wait-till-dawn clause + "You're different." left unpatched; projects-locked sv; Park Myung-soo arrival ("Ai-goo, Si-on-ah!"), the console evidence, the gapless radio refusal, the elevator-floor-1 deduction → "girlfriend getting off work" pq; she does not correct him.

**Registration & build:** opf manifest+spine ch237/ch238 (238 = spine); ncx np-241/np-242 (max 242); nav li 238. build_epub_v63.py. **22,982,674 B, 397 entries; epubcheck valid True, 0 messages.** Battery V62↔V63 sha1: exactly 3 changed (opf/ncx/nav) + 2 new chapters. Baselines (artifact-derived): chapters 238, spine/navPoints 242, nav li 238, anchors 14,170, anchors-in-blocks 0, invented-class 0, tv-screen 8, lyric-block 34, music-player 36, ls 556, sv 296, pq 138, ckl 62, pc 108, mail 13, comment-item 626, Han/fw/【】 0.

**Deep scans:** ？ from artifact ALL EXACT (234 30/29, 235 22/22, 236 11/11, 237 82/81, 238 45/45); spot checks 48/48 (2 case-variant probes re-verified); doubled-words 0; OOPS 0; misattr guards clean (Park Ji-min leakage 0; Park Myung-soo 5+17 vs Park Ji-eun 1-in-237; Choi Jin-ri ×2; bare IU 0). False-alarm logged: doctor/soldier/Cindy content greps against ch238 returned 0 — correct, the exchange lives in ch237 per raw; always locate content by RAW chapter before declaring loss.

**Delivered:** Seoul_Starting_With_Debt_Collection__Version_63.epub (V62 epub deleted at ship).

## §113 — V64 shipped (ch239–240: the red-light invitation / the bloodless conquest)

**Raw coverage (§ raws-first):** raw/chapter-239.txt (599 ln) + raw/chapter-240.txt (692 ln) saved verbatim BEFORE translation; 40+66 probes PASS (5 initial "misses" all probe artifacts: mid-quote probe strings, the &amp; entity, a title-doubling false positive — re-verified present). ？-ledger BOTH widths: ch239 **34/34/0**; ch240 **31/29/2** (fan comment 碰瓷…谁给你的勇气？×2 + the pun headline 《What Do You Mean?真正作者是谁？…》×2). The 9 title-embedded 'What Do You Mean?' ？s counted 1:1. 【】 ch239 ×1 → card sv; ch240 ×20 → comment-threads/posts/nd rows; Hangul 0.

**Defects caught by batteries this cycle:** ch240 carried ONE invented ？ — raw's 你这个表情可不像没什么。 is a statement ("Your face says otherwise."); TWO CJK slips in my own draft (热量/话题 in coined compounds — "bandwagon-riding"/"topic-worthy" restored); one CJK slip in the BUILD SCRIPT description (建议 → "suggestion") — new battery sweep: CJK ideographs across ALL content files incl. opf = 0. Self-review payoff: ch239 was EXACT on first pass.

**Canon:** all shipped forms reused EXACTLY — Park Jin-young, Yoo Jae-suk, Bae Suzy, Ji-eun-ah, PD-nim, Chungjeong-ro, Lee Joon-ik, 'La La Land', Tropical House, Met Gala, Wiz Khalifa, Girls' Generation. NEW plain: OMI, 'Cheerleader', Belieber, Wonder Girls, Yeongdong Expressway Music Festival, Byun Baek-hyun (EXO/SM main vocal; house hyphen style). Bare-IU law held inside Yoo Jae-suk's producer line: 比如IU → "Ji-eun, for instance." (a dialogue attribution of the banned form is still banned). **Raw-author arithmetic drift preserved VERBATIM:** ch239's 二十三首歌 → "the twenty-three-song debt" although ch237's ledger says 25 remaining — never silently harmonize; logged here.

**Translation & blocks (deep-thinking conversion):** ch239 "The Premiere Invitation" ls2+pc1+sv3+ckl1+tv1+pq2 — Myung-soo's strike-rushing, the two-line "Downstairs." pc, the van-inspection sv (no bags, no takeout — "Someone forbade it."), 23-song-interest banter, staff-meal plan ckl (₩10M deposit, receipts settlement, tax lecture + Ji-eun's audit joke), corporate card sv (LAVUE Staff Meal Account), the food war + mukbang offer + checkout standoff, aunt's knowing pause, tenure pq, the cinema LED wall tv-screen carrying the 'Sado' concept poster (Yeongjo above in red, Sado kneeling in white), calendar sv ("Send me an invitation"), Ji-eun-ssi / "Ne." / Film-Council boilerplate → the red-light re-do: "I hope you'll come see my film." — one person to one person — "We'll see how the schedule looks." + closing acceptance pq. ch240 "JYP's Dignity and Ji-eun's Bloodless Conquest" ls2+nd5+comment3+sv5+ckl3+pq3 — OMI smear nd ×4+…, fan counterattack thread (2-mark comment intact), first-week chart sv (OMI/Bieber/BS&Wiz), WDYM anatomy prose, Bieber-return nd, credits sv (Lyrics BS+JB / Comp BS / Prod Jung Jae-joon), detonation thread (WAIT + boomerang), dirty-PR nd (3 ？ headlines), updated chart sv, Bieber's first-career-crown backstory ('Legend' dread, 'Love Yourself' jealousy, "grows the whole pie"), thank-you post sv ×4 rows + Met Gala note, the orientation reply sv ("My orientation is perfectly straight. Either way — congratulations."), laughter thread (psychological-shadow area), pivot nd with the 2-mark pun headline, "Bro why..." out-liking the original, Scooter's single like → social-demise pq, streaming-era "moral condemnation failed to defeat the ear" pq; Pyeongchang ls; Jae-suk floor comedy ("Enjoying sitting."), America-ledger ckl (WG residency → SNSD through the door → cautionary tale), 'Dream' timeline ckl (March aegyo call → SIXTEEN shelving → summer calculus), feared-headlines folded into the ckl count row, IU's perfect-listening air-raid (Milan tea still warm; "She had invented it."), pride scaffold → "Terminator in a pink sundress" → Byun Baek-hyun ("equal marriage between two of the Big Three", "the sheer prestige of it") → "Trying contact isn't the same as guaranteeing a collaboration." → closing summer-hiring pq.

**Registration & build:** opf manifest+spine ch239/ch240 (240 = spine); ncx np-243/np-244 (max 244); nav li 240. build_epub_v64.py. **23,002,290 B, 399 entries; epubcheck valid True, 0 messages** (re-run after the description CJK fix). Battery V63↔V64 sha1: exactly 3 changed (opf/ncx/nav) + 2 new chapters. Baselines (artifact-derived): chapters 240, spine/navPoints 244, nav li 240, anchors 14,269, anchors-in-blocks 0, invented-class 0, tv-screen 9, lyric-block 34, music-player 36, ls 561, sv 304, pq 142, ckl 65, pc 109, mail 13, comment-item 642, Han/fw/【】 0, CJK ideographs 0 everywhere.

**Delivered:** Seoul_Starting_With_Debt_Collection__Version_64.epub (V63 epub deleted at ship).

## §114 — V65 shipped (ch241–242: the Norwegian bedroom producer / three gazes at the premiere)

**RAWS-FIRST INCIDENT + RECOVERY:** first write of both raw files corrupted them (placeholder tokens, paraphrases, dropped lines) — caught by immediate stray-token + restore probes, both files REWRITTEN verbatim from source; stray tokens 0, restore probes PASS (2 residual "misses" were probe typos). New law (§76.1): the raws write is the one operation where the content must be transcribed EXACTLY — verify with spot probes + token sweep IMMEDIATELY after writing, before anything else.

**Raw coverage:** raw/chapter-241.txt (671 ln) + raw/chapter-242.txt (1,031 ln). ？-ledger BOTH widths: ch241 **29/26/3** (三天？会不会太赶？/ 万一什么？万一…？ / 见到白时温…？…？ — Alan's three self-doubt ？s are narration); ch242 **46/45/1** (嗯？谢什么？). Ch241 first pass lost 3 lines by merging raw ？-lines (fold-law §74.1) → split back; one invented ？ avoided (raw's 这句话的攻击性 is a statement). 【】 ch241 ×17 → all into sv email blocks + the DJ-formula ckl; ch242 ×0; Hangul 0; CJK ideographs 0 across ALL content files.

**EPUBCHECK PRE-FLIGHT SAVED A BUILD:** Lee Sang-hyeok was drafted with chr-lee-sanghyeok/faker.jpg — grep confirmed NO card, NO jpg (the exact V61 RSC-007/012 trap) → stripped to plain BEFORE building (§72.3 pre-flight run at draft time, not after). Park Ji-hoon (stylist, Paris) IS carded → anchored; Krystal Jung carded → anchored; Choi Jin-ri carded (chr-choi-jinri + sulli.jpg) → anchored; Bae Joo-hyun (chr-irene), Lee Ji-eun (chr-lee-ji-eun) anchored.

**Canon:** Alan Walker / DJ Walkzz / 'Fade' / Bergen / FL Studio / SoundCloud / Camilla (sister) / Easyhoon / Felix Jaehn / The Chainsmokers / Zedd / DJ Snake / Billie Eilish (碧梨) / SKT T1 / 'My Love from the Star' (×6 precedent) / COEX · Megabox · Samseong-dong / Incheon / f(x) / '4 Walls' / Kim Jong-in — ALL PLAIN (Kygo has ×1 precedent); S4 opening = Seoul, Sangam Stadium, 'Legend' + BTS war-dance (frozen corpus); Lee Sang-hyeok PLAIN ×12-registered; RV order: Kim Yerim (NEVER "Kim Ye-rim") leads, Kang Seul-gi / Park Soo-young / Son Seung-wan follow, Bae Joo-hyun last; Jeon Hye-jin plain new; 大发 chants rendered in English ("'Sado' is going to be huge!").

**Translation & blocks (deep-thinking conversion):** ch241 "Faker's Deadly Temptation" ls2+sv6+ckl2+tv1+pq2 — the salon (Park Ji-hoon in Paris), the Riot reply sv (praise → "read the turning point" → the Berlin 'more diverse elements' ask), the DJ-era ckl (formula + four names + Billie Eilish kicker), DJ Walkzz spelled out, the near-anonymous SoundCloud sv, the test-brief ckl (no melody change / no leaks / three days / credit+fee+royalties), the one-line acceptance sv, the NDA-with-dad sequence, the $10,000 wire sv + "not payment — respect" pq, the S4 Sangam replay tv-screen ("still worshipped on the forums"), the untouched-sandwich sacrifice, twelve-hours-not-three, the fan-letter drafts deleted, the F5 vigil ("Confirming the network is fine."), the Seoul-invite sv, Camilla's interrogation ("That's why humanity invented meeting."), the Faker email sv ("To hell with social anxiety!"), the browser lecture (S3 / two Zeds / un-petrifiable), the acceptance sv, and the closing rank pq ("Some things, the idol is simply better off not knowing."). ch242 "Three Gazes at the Premiere" ls3+sv2+ckl1+pq2 — Incheon pickup (Eun-ah's English, "Alan is fine"), the Faker-first detour, 'My Love from the Star' disillusion + "we don't usually station aliens next to the highway", SKT T1 (double-mid Easyhoon context; "Then this year I'll work my way back." = the song's thesis; the rank question answered with silence), chili-paste defeat, the studio session, Jin-ri's midnight snack run ("Renewal depends on the quality of the late-night delivery."), the Sulli↔DJ introductions, the head-texture review ("the answer required no translation"), the sleep-carry ("You're entitled to."), the limited-edition scene ("This is my first time seeing it." ×2), the elevator "Oppa"/weight banter, the seatbelt kiss + after-sales service, the premiere invitation ("Because I want you there." / red carpet vs after-credits identities), COEX corridor sv (poster: prince will rebel, king will die; rice chest buried), fan chants, the guest-list sv (Choi Jin-ri / Lee Ji-eun / Red Velvet five — "it's over" + rice-chest pq), the family-portrait irony, the signing detour, Eun-ah's worst-case ckl (three seating plans) meeting arm-in-arm reality, the compliment-order duel ("Too?"), the hiding-from-us deduction ("Shouldn't be me." / "Nor me."), Ji-eun's peach-blossom retell (Pyeongchang sniper story + Milan prehistory + the Krystal→EXO→Baekhyun confirmation chain — 99% believed, "Thank you, unnie."), the warning ("The truly dangerous ones are usually the people who've always been nearby."), the RV entrance, and the three-gazes closing pq.

**Registration & build:** opf manifest+spine ch241/ch242 (242 = spine); ncx np-245/np-246 (max 246); nav li 242. build_epub_v65.py cloned via python (no CJK sed — §75.2). Sandbox pre-flight re-run (work symlink + pip epubcheck/jdk4py + java symlink). **23,024,958 B, 401 entries; epubcheck valid True, 0 messages.** Battery V64↔V65 sha1: exactly 3 changed (opf/ncx/nav) + 2 new chapters. Baselines (artifact-derived): chapters 242, spine/navPoints 246, nav li 242, anchors 14,408, anchors-in-blocks 0, invented-class 0, anchor pre-flight 0, tv-screen 10, lyric-block 34, music-player 36, ls 567, sv 314, pq 146, ckl 67, pc 109, mail 13, comment-item 642, Han/fw/【】 0.
...
## §119 — V70 · style-block line-separation redesign + call blocks + ch78 cleanup (fix pass, no new chapters)

**User-directed fixes:**
1. **List-block line separation (book-wide CSS).** `checklist-block .ckl-step` had `margin: 0.18rem 0`, so steps rendered as one merged paragraph; `screen-view .sv-line` (the model) uses 0.45rem. Audited every block's repeated `<p>`-row margin and raised the tight list-style blocks to clear separation: ckl-step→0.55rem; album-card .al-line, dialogue-block .dl-line, live-stage .ls-line/.ls-scene/.ls-note, stage-block .st-line/.st-scene → 0.5rem; and the `.block p` bases for dialogue-block, live-stage, stage-block, news-digest, trend-block, metric-block, acting-block, award-block, contract-block, studio-block, mail-block, announcement-block, official-post → 0.5em. 21 rules total. Left screen-view (model), pullquote/lyric (intentional), chat/comment structural bits.
2. **ch249 video call → `facetime` block.** The Baek Si-on ↔ Choi Jin-ri video call was plain prose + dialogue-lines; converted to facetime (ft-bar/ft-party/ft-them/ft-me/ft-note), names plain inside (§71.1), all 6 ？ preserved.
3. **ch250 Hyundai call → `phone-call` block.** The Genesis/Hyundai call was prose + a checklist-block; converted to phone-call (pc-head/pc-them/pc-me/pc-note), names plain inside, the 1 ？ preserved.
4. **ch78 `「」` → curly quotes.** The last 20 CJK corner brackets (around song/artist titles) converted to ‘’; **CJK/fullwidth is now 0 across all 250 chapters.**

**Wide deep scan (all 250 chapters):** fullwidth ？ in English text = 0; anchors-inside-style-blocks (§71.1) = 0; CJK/fullwidth = 0 (post ch78 fix); repeated-word/typo scan = 0 real hits (all false positives: "La La", "won on", "not noticing", "had had enough"). Call/message mentions in 35 chapters were checked — all incidental references ("after the video call ended", "a certain 1:07 a.m. phone call"), not rendered conversations, so no missing call blocks beyond the two fixed. ？-ledger vs raw (ch121–250): 53 chapters differ, almost all ±1–3 = normal English/Chinese question-mark variance (the ledger is a completeness heuristic, not an exact-match gate); 0 fullwidth-？ leaks is the concrete house-style gate and it passes.
