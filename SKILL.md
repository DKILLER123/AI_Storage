...
## §60 · V50 Rules (ch211–212 + the Hover-Preview, Host-Block, and Spine Laws)

**§60.1 Hover-preview law (bookwide).** chr-peek hover previews are allowed ONLY in plain narration text (classless `<p>`, `dialogue-line`) and the character-intro page — NEVER inside display-block divs (screen-view / phone-call / chat-container / pullquote / music-player / lyric-block and kin): images clip at the head inside blocks. The anchor `<a>` stays; only the peek `<span><img/></span>` inside it is omitted. CSS `.chr-peek` remains defined. Enforcement every cycle: in-zip scan must report PEEK-IN-BLOCKS = 0. Note the wrapper anchors classless `<p>`s inside pq bodies too — run the in-block strip AFTER wrapping new chapters, and hand-write bare anchors when drafting inside blocks.

**§60.2 Host-blocked assets.** User-supplied image URLs can be unreachable from the build sandbox (myimgs.org: DNS resolves, TLS killed pre-handshake — egress filtering; curl/wget/urllib all fail). Never substitute a different image for a blocked one: keep the current asset, report the failure precisely, and ask for the exact file via upload/attachment or an alternate host. A portrait swap ships only the user-chosen image, under the canonical filename (same filename = zero opf/nav churn; re-run epubcheck after any image change).

**§60.3 Manifest ≠ spine.** A new chapter needs THREE opf touches: manifest `<item>`, spine `<itemref>`, plus nav/ncx entries. The manifest item alone yields epubcheck RSC-011 ×2 per chapter ("reference to a resource that is not a spine item"). After registration, grep `itemref idref=` before building.

**§60.4 Battery metric keys (clarified V50).** The baseline "CJK 20" is the ten 「」 pairs in ch78 (CJK punctuation U+3000–303F, the sanctioned exception), NOT hanzi — report hanzi-only = 0. Cards/modals count keys: `class="char-intro"` and the modal-family classes (80³), not `ci-card`/`modal`. When a raw probe misses, re-verify against the raw before blaming the raw — probe typos happen.

## §61 · V51 Rules (ch213–214 + the Exhaustive-Scan, Spelling-Drift, and New-Card Laws)

**§61.1 The peek-scan block list must be EXHAUSTIVE.** V50's "PEEK-IN-BLOCKS = 0" was scan-blind, not clean: its block tuple omitted wardrobe-block / comment-thread / char-intro / live-stage, and ch125 carried 5 peeks inside wd-items. The §60.1 scan now runs against every display div type in the book (grep the shipped block census each cycle). Rule: hover previews live ONLY in plain narration text + the intro page — no exceptions, no grandfathering.

**§61.2 Spelling-drift, third strike — FORMS vs shipped register.** FORMS said "Park Ji-hun"; the shipped chapters spell "Park Ji-hoon" — the form silently matched nothing for two cycles. Before ANY new name enters FORMS or a draft: grep intro ids AND shipped chapters for the spelling; when they diverge, the SHIPPED TEXT wins, add the corrected form (retain the legacy entry), and note it in the worklog. This is the same law that caught Jung Jae-joon/Jeong Han-teuk; it now also covers romanization-variant manager names.

**§61.3 The f-string backslash trap recurs.** Three SyntaxErrors in one session (V50 §56.1 note, twice in V51): NEVER put regex literals, escaped quotes, or backslashes inside f-string expressions. Hoist every pattern/segment to a variable before the print. If a battery script dies on line N with a caret under an f-string, this is why — fix and rerun, do not hand-simulate the output.

**§61.4 In-block names: wrapper-blind rows.** The wrapper touches ONLY classless `<p>` and `dialogue-line` — sv/wd/comment floors are invisible to it. Load-bearing names in sv rows get HAND BARE anchors (ch212/214 precedent); fan-voice comment floors ship PLAIN (ch22 precedent); img alt="Portrait of X" and chi-name rows are exempt from residual sweeps. The residual sweep itself must test anchor-span membership positionally (a fixed 80-char context window false-positives on peek spans).

**§61.5 New-card pipeline (proven on Chazelle/Stone).** Generate portrait (AI; convert for size trim — PIL absent in sandbox) → verify JPEG magic + ≤300 KB → ci- card after the newest card, chr- modal after its sibling → opf `img-` item (§56.2 manifest≠zip-glob) → FORMS entries full + bare, with a collision comment for risky bares (bare "Emma" vs "Emma Watson") → ONE in-text char-intro card at first ON-PAGE appearance → retro-wrap prior plain mentions (bare inside blocks, peek in plain text) → wrapper only then runs on new chapters.

**§61.6 Attachment-delivery trap.** An announced attachment path can simply not exist, and a same-named file from an earlier failed download (0 bytes) can impersonate success. ALWAYS verify magic bytes + non-zero size before declaring an asset delivered; a right-name/zero-byte file is a failure, not a partial one. Report the precise failure and request re-delivery; never substitute (§60.2).

## §62 · V52 Rules (ch215–216 + the Head-Title, Post-Wrap-Insert, and Delivery-Channel Laws)

**§62.1 Head <title> ？ duplication.** The parity walker extracts text with minidom — the <head><title> is INCLUDED. When a chapter title itself carries ？ (「哪个女孩值得你花几千万？」), a literal translation in BOTH <title> and <h1> double-counts the mark (EN 37 vs RAW 36). Rule: the H1 carries the ? verbatim; the head <title> drops it (metadata, not narration). Watch for this on every ？-titled chapter.

**§62.2 Post-wrap insertions must match the WRAPPED text.** The wrapper rewrites the paragraphs it anchors — an insertion anchor written pre-wrap ("Rihanna's voice came from behind him") no longer exists post-wrap ("…<a…>Rihanna</a>'s voice…"). Insert new material BEFORE wrapping, or search for the wrapped form (re.search with explicit \n spans). The 4th f-string backslash SyntaxError this session re-confirms §61.3: hoist patterns; never inline them in f-string expressions.

**§62.3 Image delivery channels (verified).** The sandbox egress kills arbitrary hosts at TLS (myimgs.org, i.ibb.co, i.pinimg.com — exit 35; plain HTTP exit 52) and chat attachments may simply never land (verify with a full-filesystem sweep + magic bytes; a 0-byte same-named file is a failure). The ONE verified external channel is GitHub: github.com reachable, gh authenticated — have the user upload the asset to a gist or repo and pass the URL; fetch with gh/curl, verify JPEG magic + size, swap under the canonical filename, rebuild, epubcheck. Never substitute a lookalike (§60.2); a platform image-search result is NOT the user's image.

**§63 The `.search-bar` block (V53 law) + post-wrap peek strip.** (1) ANY scene where a character searches the internet or an app — Naver, Google, YouTube, IG explore, stock apps — gets a `.search-bar` div: `<p class="sb-engine">⌕ Engine · context</p>` + `<p class="sb-query">query text<span class="sb-caret">|</span></p>` + optional `<p class="sb-note">…</p>`. Created from Irene's ch215 query “Baek Si-on A’ST1.” (no autocomplete — the note carries the pathos). Curly apostrophes inside sb-query; the caret is a styled `|` with @keyframes sb-blink (1.2s steps), disabled under prefers-reduced-motion. (2) **Peek-strip law, hardened:** the wrapper's display-row skip-list lags the block inventory — after EVERY wrap, scan ALL block types (exhaustive §61.1 tuple) for `chr-peek` and unwrap any hits to plain text (block rows are never anchored); ch217's Kobe pq caught 2 this cycle.

**§62.4 Image-search photo route (verified V53).** When the directive is "the current image is the wrong person" (not "use MY file"), image_search + visual identity verification + ImageMagick crop to 3:4-ish, q88 -strip, under the canonical filename is the fast compliant route: Yeri = fansite photo cropped past the foreground occluder; Emma = Cannes white-blazer crop. ALWAYS `identify` after convert (silent geometry clamps), ALWAYS eyeball the crop (read_file), and reject queries that return 0 on-target results rather than shipping a near-miss. If the user later insists on their own files, §62.3 (GitHub) remains the channel.

**§64 V54 additions.** (1) **Card-text identity law:** the in-text debut card must be byte-identical to the intro-page card (chi-name/chi-role/chi-meta/chi-desc) — write it ONCE and copy it, never retype; Kim Eun-sook's desc drifted by two words between copies and failed the diff. (2) **近名陷阱：** 真理 = Jin-ri (Choi Jin-ri) and 嘉允/智恩-style Ji-eun are BOTH live canon names with similar romanizations — grep the intended name's raw hanzi before writing dialogue addresses like 「真理小姐」→"Miss Jin-ri" (self-caught ×2 in ch219). (3) **Statement-form ？, third consecutive cycle:** rhetorical-question shapes ending in 。 (「有没有可能预留给我。」) and exclamation lines (「居然是你本人打过来！」) read like questions in English — the walker is the only authority; never pre-judge punctuation by feel. (4) The post-wrap chr-peek strip inside display blocks (§63.1) fired again (ch220 pq) — treat the strip as a mandatory pass after every wrap, not an exception handler.

**§65 V54 bugfix laws.** (1) **Portrait identity law (user directive):** before generating ANY character portrait, determine real person vs OC. Real → image_search + visual identity verification + crop under the canonical filename (composite panels: identify, then crop the subject panel — verify with read_file). OC → AI generate. Never assume; Kim Eun-sook was AI-generated until the user corrected it. (2) **Chat label law (hardened §52.7):** EVERY bubble carries its own chat-name label directly above it; the POV owner's label = `chat-name self` and their bubbles = chat-sent. A received bubble under another party's name label is a MISATTRIBUTION. The battery now audits label-vs-bubble alignment bookwide (ch195 is the canonical shape). (3) **Record-accuracy check:** before recording a fix as DONE in the worklog, grep the artifact — §101 claimed the Rihanna .phone-call conversion that was never executed; the §103 deep scan caught it one cycle later. (4) A phone call narrated in summary by the raw (no quoted dialogue) does NOT owe a .phone-call block (UA call precedent).

**§66 V55 additions.** (1) **Chart-block law:** ranked lists (search suggestions, music charts) use `.naver-search` (nv-header / nv-row > nv-rank + nv-term / nv-note) — the ONLY styled chart container; a bare `nav-block` renders unstyled and is banned. (2) **Imagined-unsent chat drafts:** a message composed but never sent may still be displayed as a chat-container, with the header explicitly labeling it (e.g. "the draft, never sent"); the POV owner's label still takes `chat-name self`; predicted reactions stay plain prose (never fabricated bubbles). (3) **Quoted stage names:** narration renders 서현-stage names per canon (Seung-wan EXCLUSIVE), but a third party's QUOTED line may use the real stage name when the raw does (producer's "Wendy", ch165 precedent). (4) **Sandbox-reset recipe (updated §55.1):** recreate `ln -s /home/user/AI_Storage/work /home/user/work` AND `pip install --break-system-packages epubcheck jdk4py`; repo content survives, symlink + pip toolchain do not. (5) The battery now includes: chat label-vs-bubble alignment bookwide, chart class = naver-search, and per-block counters for ckl/chart.

## §67 · V56 Rules (ch223–224 + the Short-Name Migration and the Unify Laws)

**§67.1 Short-name law (user directive, supersedes §66.3's exclusivity clause).** Short forms are never hyphenated: Sulli, Yoona, Seulgi, Wendy — everywhere (narration, anchor labels, chat labels, FORMS short rows, dialogue). Full names (Kang Seul-gi, Son Seung-wan, Park Soo-young…) remain exactly as the raw writes them, and card display names stay full-name. When migrating a live book: two-pass protected replace (mask full names first, replace bare short forms, unmask), then verify bare-0 AND full-name-count-identical before believing the report. FORMS gains BOTH rows (full + short) pointing at the same chr-id, longest-first (full name above short).

**§67.2 Case/form unify law: mirror the raw per occurrence, then name the rule.** For a title that appears in multiple casings in the raws (OOPS/Oops), do NOT pick one bookwide — classify by context and mirror: file names + production/planning-document references keep the raw's caps (OOPS_demo_CP_v2, 《OOPS》 in campaign docs); released-title references (charts, commentary, ads as public artifacts) take the raw's title case ('Oops'); the bare word inside lyrics ships as the lyric. After unifying, grep every touched chapter to 0 residual wrong-form.

**§67.3 Derived-count slip law, hardened:** every number that enters a worklog entry (counts, deltas, totals) must be re-derived from the artifact by a stated method in the same session — never from memory of an earlier scan. The V56 cycle shipped with two memory-slips caught by re-derivation: full-name counts "59/37" (true 61/45 — plain .count) and "pq119" (true 115 under the pq-glyph pattern; both V55/V56 artifacts measure 115). Different counting methods (boundary-lookaround regex vs plain .count) legitimately disagree — state the method beside the number or don't record it.

**§67.4 Silent-owner group chats are legal.** The chat-coherence flag "(b) ≥2 parties all-received" also fires when the POV owner genuinely speaks once (e.g. drops two photos) and then goes silent while the group runs on — if each container's bubbles match the raw sender-for-sender, document the flag as intentional instead of inventing a self bubble. The invented-bubble fix would violate §66.2; the flag is a triage prompt, not a ban.

**§67.5 Scanner false-positive traps (all three bit this cycle):** (1) paragraph regexes must exclude self-closing `<p class="chat-clear"/>` — otherwise every match runs from the chat-clear to the NEXT </p> and imports container whitespace as paragraph text (ch223 inflated +25 phantom double-spaces); (2) the §54.1 framed sweep must be paragraph-scoped (text starts — AND ends —) — a cross-paragraph `—.*?—` with re.S matched one dash in ch197 and closed on a dash paragraphs later; (3) the CJK battery counts Han + fullwidth FORMS (\u4e00-\u9fff, \uff00-\uffef); CJK punctuation brackets 「」 are §66.3 stage-name canon (ch78, 10 pairs) and exempt.
...
## §71 — V60 laws (the style-block anchor purge + ls spacing + two-sided calls)

**§71.1 NO character anchors inside ANY style block (user law).** No chr-inline, no hover preview, no clickable link — plain names only — inside pullquote, screen-view, checklist-block, phone-call (incl. pc-head and in-block narration), mail-block, news-digest, sv/nd/ckl rows, chat rows, location-stamp, comment rows, wardrobe/album blocks — any div except page-wrapper. Battery: book-wide anchors-in-blocks = 0 (depth-matched scan). The purge found THREE shapes: full-shape peek anchors in pq/pc/nd (9), LEGACY no-peek anchors in old pq/sv/wardrobe blocks (43 — a repair regex requiring the chr-peek span misses them; match `<a class="chr-inline" href="[^"]*">(.*?)</a>` generally), and wrap-pass leaks into classless in-block paragraphs (18). Strip = keep inner text, drop the wrapper; peek parity self-preserves.

**§71.2 The wrap pass is now block-guarded.** wrap_v29_anchors.py computes depth-matched regions of every non-page-wrapper div and returns paragraphs inside them untouched — classless in-block narration (`<p>` inside .phone-call) and pq body `<p>`s can no longer be anchored by a re-run. Still: explicit argv (§70.2) + re-derive totals after every pass.

**§71.3 Flex `gap` is not vertical rhythm.** Some EPUB renderers drop flex gap entirely — .location-stamp's gap alone let ls-date/ls-time/ls-place collide. Any flex-column block must ALSO carry explicit margins on its children (V60: 0.34rem rhythm on ls-date/ls-time/ls-place).

**§71.4 Two-sided calls use .pc-them.** The class existed in stylesheet.css unused since the block's birth — before inventing any call markup, grep the CSS inventory (§66.2-adjacent). pc-head names both parties PLAIN (§71.1); pc-me = POV speaker; pc-them = the other party's quoted lines (first use: Eun-ah production call ch231, Kobe farewell call ch232).
...
## §77 — V66 laws (the Alan Walker card, the archaic-negation defect, and the ledger's XML-decl offset)

1. **Card parity at first carded use, real-photo law.** When a recurring real person first appears and is to be anchored (Alan Walker ch241+), add the `ci-*` card + `chr-*` modal + manifest `img-*` item + the FORMS entries in `wrap_v29_anchors.py` in the SAME pass, using a verified real photograph (downscale to ~1000px to hold the epub weight). Also audit the character-intro page for `chi-name`s left as plain text — every card must carry the `<a href="#chr-…">` link (V66 linked 15 stragglers).
2. **Archaic inverted negation is a defect class.** "…you understood not at all", "exchange-confirmed", "more lost again" are Chinglish/archaisms; on catch, repair to natural English and log. The book-wide sweep regexes for verb+not, doubled words, and article-vowel mismatches are the battery; intentional chants/lyrics/nicknames are exempt.
3. **？-ledger raw-vs-shipped must subtract the XML declaration's two '?'**. Comparing raw ？-totals to a chapter's '?' count yields a systematic +2 unless the `<?xml …?>` prologue is stripped first (ch196–242 reconcile exactly after stripping). Legacy ch121–195 deltas are title-？/comment-flood artifacts — benign, sampled, and left shipped; do not bulk-fix.
4. **Film watched on a cinema screen = `.tv-screen`** (extends §72). The on-screen dialogue of a film the cast is watching (the ‘Sado’ premiere passages) ships as tv-channel/tv-quote/tv-line blocks, never flat prose. On-stage spoken Q&A = `.interview-block`; shouted press/fan callouts = `.live-stage`; an internal tally/ledger (selective optics, a storyboarded scheme) = `.screen-view` with sv-line/sv-note.

## §78 — V67 laws (the ？-ledger as a completeness gate, and user-portrait priority)

1. **The ？-ledger is a completeness gate, not just typography.** A draft that silently compresses the raw drops question marks with everything else. ch246’s first draft shipped 12 ？ against the raw’s 30 — the ledger caught the over-compression before packaging, and the chapter was retranslated line-by-line. Rule: after drafting, ？(draft) must equal ？(raw) for the chapter; a shortfall means dropped paragraphs, never “acceptable condensing.” One-raw-para = one-EPUB-para (§76.2) is enforced by this count.
2. **User-supplied `uploads/` portraits override web photos, installed by filename-overwrite.** When the user provides their own Ariana Grande / Alan Walker images, copy them straight over the existing `OEBPS/images/<slug>.jpg` keeping the filename, so manifest/cards/modals/anchors need zero churn. Verify dimensions post-copy; downscale only if the epub weight demands it (these two were small enough to ship as-is).
3. **epubcheck’s JRE may be off-PATH but bundled.** `epubcheck` exits with `FileNotFoundError: 'java'` when no system JRE exists; the sandbox ships one at `/usr/local/lib/python3.11/dist-packages/jdk4py/java-runtime/bin`. Prepend that to PATH rather than treating validation as blocked.

## §79 — V68 laws (the chat-block audit, and display-block lines that map to raw lines)

1. **A KakaoTalk/text reply must ship as a chat-container, not a flat dialogue-line.** When the narration says a character “switched to KakaoTalk and replied,” the reply is a message — render it as a `chat-container` (chat-header “KakaoTalk · <name>”, chat-name self, chat-sent bubble), matching any earlier thread with that contact. ch78’s “I’ll get word to you in a few days…” had shipped as a straight-quote dialogue-line; converted. Audit pass: grep dialogue-lines whose preceding narration names a messaging app.
2. **Legacy straight-quote typography is book-wide in early chapters (ch21/43/66–80…) — do NOT bulk-fix.** It predates the curly-only enforcement and ships in V66–V68. Correct only lines you are already rewriting; report the rest, don’t sweep 73 chapters.
3. **Display-block child lines often map 1:1 to raw lines — count them as mapped, not invented.** `ckl-band` / `nd-source` / `sv-header` are “invented” only when they label a block that has no corresponding raw line (e.g. “the two that broke the curve”). When the band/source/header IS a raw line (Lotte’s plan = “乐天的计划是：”, the decay-curve lead-in, the Before/After revision labels), it maps 1:1. The parity script must not blanket-exclude these class names — verify per block, or the count shows a false shortfall.
4. **Never merge two raw lines into one block step.** The unboxing checklist (第一本。/偶像签名，感动。×5) is 10 raw lines = 10 ckl-steps, not 5 merged steps. Segment-by-segment parity (split on scene-breaks) catches merges the ？-ledger can’t.

## §80 — V69 laws (chr-inline is the only anchor mechanism, and the house skeleton is non-negotiable)

1. **The shipped anchor convention is `chr-inline` ONLY — never hand-write `entity-card-anchor` spans.** ch247/248 (and every shipped chapter) carry 0 `entity-card-anchor`; characters are wrapped by `wrap_v29_anchors.py` into `<a class="chr-inline" href="character-intro.xhtml#chr-…">`, and films / non-FORMS names (Fred Astaire, Gene Kelly, Jung Soojung, President Park…) stay **plain text**. Drafting ch250 with 129 hand-written `entity-card-anchor` spans was wrong on two counts: wrong mechanism, and several landed inside style blocks (§71.1). Let the wrapper do characters; write everything else plain.
2. **Romanization must match the wrapper FORMS / card id, not your ear.** The card is `chr-baek-jeonghoon` and FORMS has “Baek Jeong-hoon” — drafting “Baek Jung-hoon” silently produced an unanchored name. When a character won’t anchor, check FORMS for the exact spelling before assuming the wrapper is broken.
3. **Unwrapping a span that contains a nested anchor needs a structure-aware edit, not `(.*?)</span>`.** The naive regex matched the inner `chr-peek`’s `</span>` and produced `<span class="chr-peek"><img…/>Name</a></span>` (mismatched-tag XML failure). Repair = move `</span>` to directly after the `<img>`: `(<span class="chr-peek"><img…/>)([^<]*)(</a>)</span>` → `\1</span>\2\3`.
4. **A new chapter must be born on the house skeleton — epubcheck does NOT catch a wrong skeleton, only a missing file.** ch250 first shipped a bare `<html>` (no `<!DOCTYPE html>`), no `page-wrapper`, a plain `<h1>` instead of `chapter-header`/`chapter-number`/`chapter-title`, `style.css` instead of `fonts.css`+`stylesheet.css`, and invented block sub-classes. epubcheck flagged only the missing `style.css` (RSC-007); the invented classes (`location-stamp-label`, `ckl-head`, `screen-line`, `music-player-title`, `pullquote-text`…) pass epubcheck but render **unstyled**. Canonical sub-classes: location-stamp → ls-date/ls-time/ls-place/ls-sub; checklist-block → ckl-band + `<p class="ckl-step">` (never `<ul>`/`<li>`); screen-view → sv-header/sv-scene/sv-line/sv-caption/sv-note; music-player → mp-art/mp-title/mp-artist/mp-trackbar/mp-time; pullquote → pq-glyph + bare `<p>` + pq-cite. **Audit every drafted chapter’s classes against `stylesheet.css` (invented = 0) before wrapping** — copy the skeleton from the previous chapter, don’t retype it.
5. **The sandbox can be reset between sessions — re-verify tooling, don’t assume.** This session opened with no `java`, no `epubcheck.jar`, and the `/home/user/work` symlink gone (builder SRC unresolved). Recovery: `pip install --break-system-packages jdk4py epubcheck` (epubcheck 5.3.0 bundles the jar; run with jdk4py’s `java` on PATH + JAVA_HOME), recreate the symlink, and point the builder’s `SRC` at the real path `/home/user/AI_Storage/work/epub_src` so it no longer depends on the symlink. The github release-assets CDN was SSL-blocked; the pip wrapper’s bundled jar was the working path.

## §81 — V70 laws (call blocks, list-block separation, and verifying uploads exist)

1. **A rendered conversation must use its call block, not prose.** A video call ships as `facetime` (ft-bar “Video call/FaceTime · <name>”, ft-party, ft-them = the other party, ft-me = the MC, ft-note = action); an audio call ships as `phone-call` (pc-head “<caller> → <recipient> · context”, pc-them/pc-me/pc-note). Names inside are **plain text** (§71.1 — zero anchors in any block); preserve every ？. ch249’s Sulli video call and ch250’s Hyundai call had both shipped as prose + dialogue-lines; converted. Scan signal: a passage with “video call / phone call / the phone rang” followed by a run of dialogue-lines and no facetime/phone-call block.
2. **List-style blocks need real inter-line margin or they read as one paragraph.** The model is `screen-view .sv-line` (0.45rem). Any block whose repeated `<p>` rows sit at ≲0.35rem merge visually. Audit: for each block, find the `<p class>` that repeats as a sibling, read its effective margin (remember `.block .child` beats `.block p`), and raise the tight ones to ~0.5rem/0.5em. Fixed: checklist-block, album-card, dialogue-block, live-stage, stage-block, news-digest, trend-block, metric-block, acting-block, award-block, contract-block, studio-block, mail-block, announcement-block, official-post. Do NOT touch pullquote/lyric-block (intentional tight) or chat/comment structural children (name/meta are bubble parts, not list rows).
3. **`「」` are CJK and violate the zero-CJK law — convert to curly quotes.** ch78’s corner brackets around titles became ‘’. After this, CJK/fullwidth = 0 across all 250 chapters. The ？-ledger (raw vs EPUB) will show ±1–3 diffs in ~53 chapters from English/Chinese question-mark variance — that is not an error; the hard gate is 0 fullwidth ？ in the English text.
4. **Verify a user-reported upload actually exists before relying on it — and never substitute a web image.** The user said `uploads/Damien.jpg` was added; it was not in the local workspace because it had been pushed straight to GitHub (`Add files via upload` commit) — so **check the remote branch, not just the local tree**, when a reported upload is missing. It came in on rebase (f521910); installed over `images/damien-chazelle.jpg` (the card/character-intro already point there; `object-fit: cover` absorbs the aspect). No web substitute was used (standing constraint).

## §82 — V71 laws (list rows copy sv-line's STRUCTURE not its color; shoots get slates)

1. **A list row's separation should copy `sv-line`'s bar implementation, not its palette.** What makes sv-line rows distinct is padding + tinted gradient background + 3px left border + rounded right edge + bottom margin. Apply that structure to `.ckl-step` (and similar rows) but recolour to the block's own accent (checklist = cyan #6fc9de / bg #101c2e / text #eaf4fb), never sv-line's gold #d4af37 / cream #f6ecc9. A plain margin bump alone still reads as one paragraph on some renderers.
2. **Movie-shooting scenes get a `slate-block` (the book's film-shoot block) when the raw supplies shoot context** — sl-slate (film · scene · the take), sl-title, sl-mark (handwritten constraint), sl-line (action beats), sl-action "Action", sl-note (the outcome). Rehearsal/arrival chapters (ch249) get none; shoot chapters (ch250) get one per distinct shoot. Names plain inside (§71.1); add no ？ so the ledger is untouched.
